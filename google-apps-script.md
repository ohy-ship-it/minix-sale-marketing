...
| `NT(쇼핑스토리)` | 랜딩링크 + **`&`**`nt_source` · `nt_medium` · `nt_detail` — 파라미터가 이미 붙은 주소용 |
...
화면(파이어베이스 호스팅)은 정적 파일이라 그 자체로 문을 걸 수 없습니다. 두 겹으로 막습니다.

1. **화면** — 구글 로그인을 하지 않으면 아무것도 보여 주지 않습니다.
   파이어베이스 콘솔 → Authentication → Sign-in method → **Google 사용 설정** 을 켜면 그 순간부터 걸립니다
   (앱을 다시 배포할 것 없습니다. 켜기 전에는 문을 걸지 않아 팀이 갇히지 않습니다).
2. **시트** — `doPost` 가 요청마다 `loginCheck_` 로 로그인 표(ID 토큰)를 구글에 물어 확인합니다.
   `@athomecorp.com` 계정이 아니면 거절합니다. 웹앱 주소가 새어도 소용이 없습니다.

**켜는 차례를 지켜야 합니다** — ① 콘솔에서 구글 로그인 켜기 → ② 이 `Code.gs` 를 붙여넣고 새 버전으로 배포.
거꾸로 하면 화면이 아직 표를 보내지 않는데 시트가 표를 요구해 잠깁니다.

- 액세스 권한은 그대로 **"모든 사용자"** 로 둡니다. 로그인은 표로 확인하므로 도메인 제한을 걸 필요가 없습니다
  (도메인 제한을 걸면 브라우저가 로그인 페이지로 튕겨 요청이 아예 못 갑니다 — 아래 참고).
- **사람이 아닌 것**(내 PC 에서 도는 네이버 적재 `get_gfa_report.py`)은 열쇠로 통과합니다.
  스크립트 속성 **`PUSH_KEY`** 에 아무 긴 글자를 넣고, 같은 글자를 그 PC 의 `minix_push_key.txt`
  (또는 환경변수 `MINIX_PUSH_KEY`) 에 둡니다. 이 파일은 git 에 담기지 않습니다.
- **급할 때 끄는 법**: 스크립트 속성 **`REQUIRE_LOGIN`** 을 `off` 로 둡니다. 다시 배포할 것 없이 바로 풀립니다.
- 다른 워크스페이스가 이 화면을 틀(iframe)로 끌어다 쓰는 곳(`?embed=1`)은 브라우저가 칸마다 로그인을
  따로 보기 때문에 우리 로그인이 보이지 않습니다. 그래서 그 화면에는 **[미닉스 계정으로 확인]** 단추를 둡니다 —
  누르면 우리 사이트로 직접 열리는 작은 창(`embed-login.html`)이 뜨고, 이미 로그인돼 있으면 아무것도 묻지 않고
  표와 갱신 열쇠를 건네고 닫힙니다. 열쇠는 그 칸에 담아 두므로 **다음부터는 창도 뜨지 않습니다**
  (로그인을 다시 하는 것이 아닙니다).
