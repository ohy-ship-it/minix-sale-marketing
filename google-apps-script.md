...
> 배포된 주소는 이미 코드에 기본값으로 들어가 있고 **적재까지 확인된 상태**입니다.
> 아래 과정은 **다른 주소로 바꿀 때만** 필요합니다.
...
앱에서 시트로 보내는 항목이 늘어나면 `Code.gs` 도 함께 바꿔야 합니다.
현재 항목: **파일명 · 매체 · 메시지 유형 · 최종행사명 · 발번시각**

바꾸는 법: [Code.gs](Code.gs) 를 다시 복사해 붙여넣고 저장 →
**배포 → 배포 관리 → 편집(연필) → 버전: 새 버전 → 배포**

> 배포를 새로 하기 전까지는 앱이 보낸 새 항목(매체)이 시트에 기록되지 않고 나머지만 들어갑니다.
> 이미 만들어진 헤더 행은 자동으로 바뀌지 않으니, 첫 행을 지우거나 직접 고쳐 주세요.
