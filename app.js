...
if (creativeBoard) {
  ...
  // 카드 한 장으로 가는 링크 — 주차 링크 뒤에 카드 번호를 붙인다 (열면 그 카드가 펼쳐진다)
  const cardLink = (weekId, cardId) => `${window.location.origin}${window.location.pathname}#creative-planning/${weekId}/${cardId}`;
    ...
    board.innerHTML = STATUS_OPTIONS.map(([status]) => {
      ...
      const cards = group.map((row) => `
        <article class="notion-card" draggable="true" data-id="${row.id}"
          title="끌어서 옮기기 (칸 · 차례) · 오른쪽 클릭 — 복제 · 링크 복사 · 삭제">
  ...
  // 카드를 그대로 한 장 더. 배열 · 날짜는 **새로 만들어** 담는다 —
  // normalize 는 배열을 그대로 물려주므로, 복사본과 원본이 같은 배열을 보게 된다
  // (한쪽에서 매체를 지우면 다른 쪽에서도 사라진다).
  const cloneRow = (row) => normalize({
    ...row,
    id: '',
    name: `${row.name || '제목 없음'} (사본)`,
    media: row.media.slice(),
    owners: row.owners.slice(),
    sku: row.sku.slice(),
    eventDate: row.eventDate ? { ...row.eventDate } : null,
  ...
  // 카드를 오른쪽 클릭하면 복제 · 링크 복사 · 삭제가 뜬다 (카드 밖에서는 브라우저 메뉴 그대로).
  // 메뉴는 달력들이 쓰는 공용 cardMenu 라 생김새 · 손놀림이 같다.
  board.addEventListener('contextmenu', (event) => {
    ...
    cardMenu(event, [
      ...
      if (what === 'twin') {
        ...
        const twin = cloneRow(row);
        rows.splice(at < 0 ? rows.length : at + 1, 0, twin);   // 원본 바로 뒤에 둔다
        save();
        renderBoard();
        flashNote('복제했습니다');
        return;
      ...
      if (what === 'link') {
        copyText(cardLink(currentWeekId, row.id))
          .then(() => flashNote('링크를 복사했습니다'))
          .catch(() => flashNote('복사를 못 했습니다 — 주소창의 주소를 그대로 쓰세요'));
        return;
      }
      if (what === 'drop') {
        if (!window.confirm(`'${row.name || '제목 없음'}' 카드를 지울까요?`)) return;
        const gone = rows.findIndex((one) => one.id === row.id);
        if (gone >= 0) rows.splice(gone, 1);
  ...
  /* 카드 끌어 옮기기 — 칸(상태)뿐 아니라 **그 칸 안의 차례**까지 바꾼다.
     예전에는 칸만 바꾸고 자리는 rows 에 담긴 차례 그대로였다. 급한 것을 위로 올리려면
     지우고 다시 만드는 수밖에 없었다. */
  let draggingId = null;

  // 커서가 어느 카드의 **위쪽 절반**에 있으면 그 앞에 넣는다. 아무 데도 안 걸리면 맨 뒤.
  // 끌고 있는 카드는 세지 않는다 (자기 자신 앞에 넣는 셈이 되어 자리가 안 움직인다).
  const dropSpot = (column, y) => {
    const cards = [...column.querySelectorAll('.notion-card:not(.is-dragging)')];
    for (let at = 0; at < cards.length; at += 1) {
      const box = cards[at].getBoundingClientRect();
      if (y < box.top + box.height / 2) return cards[at].dataset.id;
    }
    return null;
  };

  const clearMarks = () => {
    board.querySelectorAll('.nc-drop').forEach((one) => one.classList.remove('nc-drop'));
    board.querySelectorAll('.nc-drop-end').forEach((one) => one.classList.remove('nc-drop-end'));
  };

  /* rows 는 week.rows 를 **그대로 가리킨다** — 여기서 splice 하면 시트에도 그 차례로 담긴다.
     새 배열을 만들어 rows 에 넣으면 그 연결이 끊겨 저장이 안 된다.
     바뀐 것이 없으면 false 를 준다 (제자리에 놓았을 때 괜히 저장하지 않으려고). */
  const moveCard = (id, status, beforeId) => {
    const at = rows.findIndex((one) => one.id === id);
    if (at < 0) return false;
    const mark = (list, kind) => list.map((one) => one.id).join('|') + '#' + kind;
    const was = mark(rows, rows[at].status);

    const [row] = rows.splice(at, 1);
    row.status = status;
    let to = beforeId ? rows.findIndex((one) => one.id === beforeId) : -1;
    if (to < 0) {
      // 그 칸의 마지막 뒤에 넣는다. 칸이 비어 있으면 맨 뒤로.
      let last = -1;
      rows.forEach((one, index) => { if (one.status === status) last = index; });
      to = last + 1;
    }
    rows.splice(to, 0, row);
    return was !== mark(rows, status);
  };
  ...
  board.addEventListener('dragend', (event) => {
    event.target.closest('.notion-card')?.classList.remove('is-dragging');
    draggingId = null;
    clearMarks();
  });
  board.addEventListener('dragover', (event) => {
    ...
    event.preventDefault();          // 여기는 받는 자리라고 브라우저에 알린다
    const before = dropSpot(column, event.clientY);
    clearMarks();
    if (before) {
      const card = column.querySelector(`.notion-card[data-id="${CSS.escape(before)}"]`);
      if (card) card.classList.add('nc-drop');
    } else {
      column.classList.add('nc-drop-end');
    }
  });
  board.addEventListener('drop', (event) => {
    ...
    draggingId = null;
    clearMarks();
    if (moveCard(id, column.dataset.status, before)) commit();
  });
  ...
  const linked = window.location.hash.startsWith('#creative-planning/')
    ? window.location.hash.slice('#creative-planning/'.length).split('/') : [];
  const linkedId = linked[0] || '';
  if (linkedId && weeks.some((week) => week.id === linkedId)) {
    openWeek(linkedId);
    // 카드까지 적혀 있으면 그 카드를 펼친다 (없는 카드면 주차만 열고 만다)
    if (linked[1] && rows.some((row) => row.id === linked[1])) openPeek(linked[1]);
  }
}
...
// 그 화면이 처음 보일 때 한 번만 부른다 (안 열면 아예 안 부른다).
// 주소에 해시가 붙어 바로 열리는 경우도 있어, 지금 이미 보이면 그 자리에서 부른다.
const openViewOnce = (section, run) => {
  if (!section) return;
  let done = false;
  const go = () => {
    if (done || section.hidden) return;
    done = true;
    run();
  };
  new MutationObserver(go).observe(section, { attributes: true, attributeFilter: ['hidden'] });
  go();
};
...
// ── 엑셀처럼 수식 세기 ────────────────────────────────────────────
// 칸에 '=10*2' 라고 적으면 20 으로 센다. 종이에 계산기를 두드려 옮겨 적던 일을 없앤다.
// **Function 에 넘기기 전에 글자를 거른다.** 숫자 · + - * / ( ) · 소수점만 남기므로
// 사람이 무엇을 적어도 그것이 코드가 되어 돌아가지 않는다.
const numFormula = (text) => String(text ?? '').trim().charAt(0) === '=';

// 못 세면 null 을 준다 (적다 만 수식이다 — 앞서 센 값을 그대로 두려고 가른다)
const numValue = (text) => {
  const raw = String(text ?? '').trim();
  if (!raw) return 0;
  if (raw.charAt(0) !== '=') return parseInt(raw.replace(/[^0-9]/g, ''), 10) || 0;
  const body = raw.slice(1).replace(/[,\s\u20a9원]/g, '');
  if (!body || !/^[0-9+\-*/().]+$/.test(body)) return null;
  try {
    // eslint-disable-next-line no-new-func
    const found = Function('"use strict";return (' + body + ')')();
    return Number.isFinite(found) ? Math.round(found) : null;
  } catch (error) {
    return null;
  }
};
...
const PERF_SOURCES = {
  ...
  naverSa: {
    ...
    note: '검색광고 API 로 노출 · 클릭 · 전환을 바로 받습니다. '
      ...
      + '(<b>부가세 포함</b>으로 적고, 표에는 공급가가 들어갑니다 — 부가세를 뺀 금액). '
...
// 네이버 GFA 의 보고서 광고비와 브랜드검색에 적어 둔 금액은 부가세를 포함한 값이라,
// Code.gs 가 공급가(÷1.1)로 바꿔 내려 준다. 화면에도 그 사실을 적어 둔다 —
// 매체 화면 숫자와 다르게 보일 때 왜 그런지 알 수 있어야 한다.
// (메타 · 구글 · 카카오모먼트는 매체가 준 값 그대로다)
const PERF_NET_SOURCES = ['naver', 'naverSa'];
const PERF_NET_TEXT = '광고비는 부가세를 뺀 공급가입니다 (÷1.1)';
...
/* 전매체 검색을 **제품으로 먼저 좁힌다.**
   광고그룹 이름에는 행사만 들어 있고 제품은 **캠페인 이름**에 들어 있다
   ('google_미닉스 더 플렌더(mini)_purchase'). 그래서 제품은 캠페인으로 고르고,
   그 안에서 검색어로 광고그룹을 고른다 — '더 플렌더 체크 → 카카오 검색' 이 그 뜻이다.

   이름을 적는 방식이 사람마다 다르다 — '더 플렌더' · '더플렌더' · '플렌더' · 'Flender'.
   그래서 견줄 때 **띄어쓰기 · 가운뎃점 · 밑줄 · 붙임표를 다 털고** 소문자로 맞춘다.
...
const CROSS_PRODUCTS = [
  ...
  { id: 'dryer', name: '건조기', words: ['건조기', 'dryer'] },
];

const crossTrim = (text) => String(text || '').toLowerCase()
  .replace(/[\s·ㆍ・_\-]/g, '');

// 고른 제품의 찾을 말을 한 벌로 편다 (한 줄마다 다시 펴지 않게 미리 만들어 넘긴다)
const crossProdWords = (picked) => CROSS_PRODUCTS
  .filter((one) => (picked || []).indexOf(one.id) >= 0)
  .reduce((into, one) => into.concat(one.words), [])
  .map(crossTrim);

/* 이 줄이 고른 제품의 것인가. 아무것도 안 골랐으면 다 본다.
   캠페인 이름으로 견주되 두 가지는 광고그룹 이름도 본다.
     · 네이버 검색광고 — 캠페인을 브랜드 키워드로 짓고 제품을 안 적는다
     · 캠페인 이름이 아예 안 온 줄 — 안 그러면 통째로 사라진다 */
const crossProdHit = (key, row, words) => {
  if (!words || !words.length) return true;
  const camp = crossTrim(row.campaignName || '');
  const also = (key === 'naverSa' || !camp) ? crossTrim(row.name || '') : '';
  return words.some((one) => (camp && camp.indexOf(one) >= 0) || (also && also.indexOf(one) >= 0));
};
...
if (filenameTool) {
  ...
  if (syncEvent()) save();
  render();
  // 시트의 마지막 번호를 미리 받아 둔다 (발번을 기다리지 않게).
  // 화면을 열 때 받는다 — 페이지가 뜨자마자 받으면 다른 화면이 그 뒤에 줄을 선다.
  openViewOnce(filenameTool, pullSeq);
}
...
if (weeklyWrite) {
  ...
  let more = {};
  // 뺀 인원. 정해 둔 넷은 줄을 지워도 이 목록에서 다시 살아나므로 따로 적어 둔다.
  // 시트의 설정 줄(주 '__roster' · 파트 'hidden')에 담아 다른 사람 화면에서도 같이 빠지게 한다.
  const ROSTER = '__roster';
  let gone = [];
  const goneWait = {};                         // 방금 뺀 사람 (늦게 온 불러오기가 되살리지 않게)
  ...
  const parts = () => PARTS.concat(Object.keys(more).map((key) => ({ key: key, name: more[key] })))
    .filter((one) => gone.indexOf(one.key) < 0);
  // 뺀 사람들 (되돌리기 단추에 쓴다)
  const goneList = () => gone.map((key) => ({
    key: key,
    name: (PARTS.find((one) => one.key === key) || {}).name || myNames[key] || more[key] || key,
  }));
  const nameOf = (key) => (parts().find((one) => one.key === key) || {}).name
    || (PARTS.find((one) => one.key === key) || {}).name || myNames[key] || more[key] || key;
  ...
  try {
    ...
    if (kept && typeof kept === 'object') {
      ...
      week = kept.week || '';
      myNames = kept.names || {};
      gone = Array.isArray(kept.gone) ? kept.gone : [];
  ...
  const cache = () => {
    try {
      localStorage.setItem(KEY, JSON.stringify({ book: book, week: week, names: myNames, gone: gone }));
    } catch { /* 거들기다 */ }
  };
  ...
  // 뺀 인원 목록을 시트 설정 줄에 담는다 (다른 사람 화면에서도 같이 빠지게)
  const saveGone = () => {
    cache();
    return askSheet({ action: 'weeklyPut', week: ROSTER, part: 'hidden', name: '뺀 인원',
      text: gone.join(','), status: 'wip', by: '' })
      .catch((reason) => { note = `인원 설정을 시트에 못 담았습니다 — ${reason.message}`; tell(); });
  };
  ...
  const pull = () => askSheet({ action: 'weeklyGet' })
    .then((body) => {
      ...
      (body.rows || []).forEach((row) => {
        ...
        if (!at) return;
        if (at === ROSTER) {                    // 주가 아니라 설정 줄이다
          if (row.part === 'hidden') hidden = String(row.text || '').split(',').map((one) => one.trim()).filter(Boolean);
          return;
        }
        got[at] = got[at] || {};
      ...
      more = named;
      // 뺀 인원은 시트 값을 따른다. 단, 지금 이 브라우저에서 막 뺀 사람은 지킨다 (늦게 온 답이 되살리지 않게)
      if (hidden) gone = [...new Set(hidden.concat(gone.filter((key) => goneWait[key])))];
      if (!parts().some((one) => one.key === part)) part = (parts()[0] || PARTS[0]).key;
  ...
  const tree = () => `<aside class="wk-tree">
    ...
    return `<button type="button" class="wk-node${part === one.key ? ' is-on' : ''}" data-part="${one.key}">
          ...
          <em class="wk-out" role="button" tabindex="0" data-out="${one.key}"
      ...
      ${goneList().length ? `<div class="wk-tree-label">뺀 인원</div>
        ${goneList().map((one) => `<button type="button" class="wk-back" data-back="${one.key}"
          title="다시 넣습니다"><i data-lucide="rotate-ccw"></i>${escapeHtml(one.name)} 되돌리기</button>`).join('')}` : ''}
    </aside>`;

  const pane = () => {
    if (!parts().length) {                      // 인원을 다 뺐다
      return `<section class="wk-pane"><p class="wk-none">인원이 없습니다.
        왼쪽 <b>인원 추가</b> 나 <b>되돌리기</b> 로 한 명을 넣어 주세요.</p></section>`;
    }
    if (part === 'all') {
  ...
  const openWeek = (step) => {
    ...
    if (!book[key]) {
      const first = (parts()[0] || PARTS[0]).key;
      book[key] = { [first]: { text: '', status: 'wip', at: '' } };
      week = key;
      part = first;
      render();
      save(first);                             // 줄을 하나 만들어 그 주를 열어 둔다
      return;
    }
    week = key;
    render();
  ...
  weeklyWrite.addEventListener('click', (event) => {
    ...
    if (out) {
      ...
      delete more[key];
      delete myNames[key];
      if (gone.indexOf(key) < 0) gone.push(key);   // 정해 둔 넷도 이렇게 빠진다 (다시 살아나지 않게)
      goneWait[key] = true;                       // 늦게 오는 불러오기가 되살리지 않게
      if (part === key) part = (parts()[0] || PARTS[0]).key;
      cache();
      render();
      Promise.all([
        ...
        .then((got) => { note = `${who} 를 뺐습니다 (줄 ${(got[0] || {}).removed || 0}개)`; })
        .catch((reason) => { note = `시트에서 못 뺐습니다 — ${reason.message}`; })
        .then(() => { delete goneWait[key]; tell(); });
      return;
    }

    const back = event.target.closest('[data-back]');
    if (back) {                                 // 뺐던 인원 되돌리기
      const key = back.dataset.back;
      const who = nameOf(key);
      gone = gone.filter((one) => one !== key);
      if (!PARTS.some((one) => one.key === key)) {
        more[key] = who;                        // 나중에 넣었던 사람이면 이름을 되살린다
        myNames[key] = who;
      }
      book[week] = book[week] || {};
      book[week][key] = book[week][key] || { text: '', status: 'wip', at: '' };
      part = key;
      render();
      saveGone().then(() => { note = `${who} 를 되돌렸습니다 (전에 적은 글은 지워졌습니다)`; tell(); });
      save(key);
      return;
    }
    ...
    if (event.target.closest('.wk-add-part')) {
      ...
      const came = goneList().find((one) => one.name === who);
      if (came) {                               // 뺐던 사람 이름을 다시 적었다 — 되돌린다
        gone = gone.filter((one) => one !== came.key);
        if (!PARTS.some((one) => one.key === came.key)) { more[came.key] = who; myNames[came.key] = who; }
        book[week] = book[week] || {};
        book[week][came.key] = book[week][came.key] || { text: '', status: 'wip', at: '' };
        part = came.key;
        render();
        saveGone();
        save(came.key);
        flashNote(`${who} 를 되돌렸습니다`);
        return;
      }
...
// ── 광고자동 세팅 · 카카오 광고 세팅 ────────────────────────────────
// 행사명만 적으면 시트에서 캠페인명 · 광고그룹명 · 랜딩 URL 을 가져오고,
// 소재 파일을 골라 [만들기] 를 누르면 여기서 바로 카카오모먼트에 올린다.
// (예전 메타처럼 PowerShell 을 내려받지 않는다 — 그림은 브라우저가 읽어
...
if (kakaoSetup) {
  const STORAGE_KEY = 'minix-kakao-setup-v2';
  const HISTORY_KEY = 'minix-kakao-setup-history-v2';

  // 행사명 → 캠페인명 · 광고그룹명 · 광고명 · 랜딩 URL 이 든 시트 (파일명 도구가 적재하는 그 시트)
  const SHEET_ID = '1-IrBGbuQmcQ9Za1LCZKfV6XUaGIV5gtut5npHw0Gu_E';
  ...
  /* 랜딩 URL 을 만든다.
     ...
     LINK(GA) 처럼 http 로 시작하는 칸은 그 자체가 주소라 그대로 쓴다. */
  const urlOf = (row) => {
    const base = tr(row.landing);
    if (state.urlType === 'landing') return base;
    const tail = tr(row[state.urlType]);
    if (!tail) return base;
    if (/^https?:\/\//i.test(tail)) return tail;
    if (!base) return '';
    const join = tail.charAt(0) === '?' && base.indexOf('?') >= 0 ? `&${tail.slice(1)}` : tail;
  ...
  const rowOf = (head, cells) => {
    ...
    const cell = (name) => { const i = at(name); return i >= 0 ? tr(cells[i]) : ''; };
    const out = {};
    Object.keys(COL).forEach((key) => { out[key] = cell(COL[key]); });
    return out;
  };
  ...
  const lookupSheet = async () => {
    ...
    if (!tables.some((table) => table.length)) {
      lookup = { state: 'error', message: '시트를 읽지 못했습니다 — 시트 공유 설정을 확인해 주세요.' };
      return render();
    }
    ...
    rows = found;
    if (found.length) {
      lookup = { state: 'done', message: '' };
      // 캠페인명 · 광고그룹명은 시트 값으로 채운다 (고쳐 쓸 수 있다)
      if (found[0].campaign) state.campaignName = found[0].campaign;
  ...
  const loadBase = (refresh) => {
    ...
    return askSheet({ action: 'kakaoDefaults', account: state.account,
      ...
      .then((body) => {
        ...
        if (!tr(state.altText) && sample.altText) state.altText = sample.altText;
  ...
  const matchPicks = () => {
    picks.forEach((pick) => {
      const hit = rows.find((row) => row.file && pick.name.toLowerCase().indexOf(row.file.toLowerCase()) >= 0);
      pick.row = hit || (rows.length === 1 ? rows[0] : null);
    });
  };

  const sizeOf = (file) => new Promise((done) => {
    const url = URL.createObjectURL(file);
    const image = new Image();
    image.onload = () => { done({ width: image.naturalWidth, height: image.naturalHeight }); URL.revokeObjectURL(url); };
    image.onerror = () => { done({ width: 0, height: 0 }); URL.revokeObjectURL(url); };
    image.src = url;
  });

  const addFiles = async (list) => {
    const files = Array.from(list || []).filter((one) => /\.(png|jpe?g)$/i.test(one.name));
    for (const file of files) {
      if (picks.some((one) => one.name === file.name && one.size === file.size)) continue;
      const box = await sizeOf(file);
      picks.push({ file: file, name: file.name, size: file.size,
        width: box.width, height: box.height, row: null });
    }
    matchPicks();
    render();
  };
  ...
  const sizeBad = (pick) => {
    if (isBoard() && (pick.width !== 1029 || pick.height !== 258)) {
      return `${pick.width}×${pick.height} — 비즈보드는 1029×258 이어야 합니다`;
    }
    if (!pick.width) return '그림을 읽지 못했습니다';
    if (pick.size > 500000) return `${Math.round(pick.size / 1024)}KB — 500KB 를 넘습니다`;
    return '';
  };

  const nameOf = (pick) => (pick.row && pick.row.ad ? pick.row.ad : pick.name.replace(/\.[^.]+$/, ''));
  const urlFor = (pick) => (pick.row ? urlOf(pick.row) : '');

  const readBase64 = (file) => new Promise((done, fail) => {
    const reader = new FileReader();
    reader.onload = () => {
  ...
  const problems = () => {
    ...
    else if (!good.length) list.push('올릴 수 있는 소재가 없습니다 — 규격과 랜딩 URL 을 확인하세요.');
    return list;
  };
  ...
  const makeAll = async () => {
    ...
    work = { running: true, done: 0, total: good.length, lines: [], failed: 0, ids: null };
    render();
    ...
    try {
      ...
      work.running = false;
      work.lines.push({ bad: true, text: `캠페인 · 광고그룹을 만들지 못했습니다 — ${reason.message}` });
      return render();
    }

    (made.log || []).forEach((text) => work.lines.push({ bad: /주의/.test(text), text: text }));
    work.ids = { campaign: made.campaign, group: made.group };
    render();

    // 소재는 하나씩 차례로 올린다 (카카오가 1초에 한 번만 받는다)
    for (const pick of good) {
      try {
        const image = await readBase64(pick.file);
        const answer = await askSheet({
          action: 'kakaoCreative',
          account: state.account,
          group: made.group,
          format: FORMAT[state.kind],
          name: nameOf(pick),
          altText: tr(state.altText),
          landing: urlFor(pick),
          fileName: pick.name,
          image: image,
          title: tr(state.title),
          description: tr(state.description),
          // 요청 이름(action)과 부딪히지 않게 다른 이름으로 보낸다
          actionButton: state.action,
          profileName: tr(state.profileName),
          profileImageUrl: sampleImage(),
        });
        work.lines.push({ bad: false, text: `${nameOf(pick)} — 올렸습니다 (소재 ${answer.creative || '?'} · 심사 대기)` });
      } catch (reason) {
        work.failed += 1;
        work.lines.push({ bad: true, text: `${nameOf(pick)} — 실패: ${reason.message}` });
      }
      work.done += 1;
      render();
    }
    ...
    work.running = false;
    history = [{ group: tr(state.groupName), kind: KIND_MEDIA[state.kind],
      at: new Date().toLocaleString('ko-KR'),
      campaignId: made.campaign, groupId: made.group,
      ok: work.total - work.failed, bad: work.failed }, ...history].slice(0, 20);
    saveHistory();
    render();
  };
  ...
  const sheetTable = () => {
    ...
    if (lookup.state === 'error') return `<p class="setup-alert">${escapeHtml(lookup.message)}</p>`;
    if (lookup.state === 'idle') return '';
    if (!rows.length) return `<p class="tool-empty">${escapeHtml(lookup.message || '일치하는 줄 없음')}</p>`;
    const empty = rows.filter((row) => !urlOf(row)).length;
    return `<div class="tool-table-wrap"><table class="tool-table setup-table">
      <thead><tr><th>파일명</th><th>광고명</th><th>${escapeHtml(urlTypeLabel())}</th></tr></thead>
      <tbody>${rows.map((row) => `<tr>
        <td><span class="setup-code">${escapeHtml(row.file)}</span></td>
        <td>${escapeHtml(row.ad) || '<span class="tool-blank">-</span>'}</td>
        <td class="setup-url">${escapeHtml(urlOf(row)) || '<span class="tool-blank">비어 있음</span>'}</td>
      </tr>`).join('')}</tbody>
    </table></div>
    ${empty ? `<p class="setup-alert">랜딩 URL 이 비어 있는 줄이 ${empty}개 있습니다 — 시트의 <b>랜딩링크</b> 칸을 채우거나 URL 유형을 바꿔 주세요.</p>` : ''}`;
  };

  const picksBox = () => {
    if (!picks.length) return '<p class="tool-empty">아직 고른 소재가 없습니다.</p>';
    return `<div class="tool-table-wrap"><table class="tool-table setup-table">
      <thead><tr><th>파일</th><th>크기</th><th>올라갈 이름</th><th>랜딩 URL</th><th></th></tr></thead>
      <tbody>${picks.map((pick, i) => {
    const bad = sizeBad(pick);
    const url = urlFor(pick);
    return `<tr${bad || !url ? ' class="is-bad"' : ''}>
        <td><span class="setup-code">${escapeHtml(pick.name)}</span></td>
        <td>${pick.width}×${pick.height} · ${Math.round(pick.size / 1024)}KB${bad ? `<br><small class="setup-warn">${escapeHtml(bad)}</small>` : ''}</td>
        <td>${escapeHtml(nameOf(pick))}</td>
        <td class="setup-url">${escapeHtml(url) || '<span class="setup-warn">짝지을 줄이 없습니다</span>'}</td>
        <td><button type="button" class="tool-media-remove" data-drop="${i}">×</button></td>
      </tr>`;
  }).join('')}</tbody>
    </table></div>`;
  };
  ...
  const workBox = () => {
    ...
    return `<div class="setup-done">
      <div class="setup-done-head">
        <span class="setup-done-name"><b>${work.running ? '만드는 중…' : '끝났습니다'}</b>
          <small>${ids ? `캠페인 ${escapeHtml(ids.campaign)} · 광고그룹 ${escapeHtml(ids.group)} · ` : ''}소재 ${work.done}/${work.total}${work.failed ? ` · 실패 ${work.failed}` : ''}</small></span>
      </div>
      <ul class="setup-hist">${work.lines.map((line) => `<li${line.bad ? ' class="is-bad"' : ''}><b>${line.bad ? '⚠' : '✓'}</b><small>${escapeHtml(line.text)}</small></li>`).join('')}</ul>
      ${work.running ? '' : `<p class="setup-note">광고그룹은 <b>꺼진 상태</b>입니다. 카카오모먼트에서 소재 심사 결과를 확인하고 켜 주세요.</p>`}
    </div>`;
  };

  const issuesBox = () => {
    const list = attempted && !work ? problems() : [];
    if (!list.length) return '';
    return `<div class="setup-issues"><b>아래를 먼저 채워주세요</b><ul>${list.map((issue) => `<li>${escapeHtml(issue)}</li>`).join('')}</ul></div>`;
  };
  ...
  const render = () => {
    ...
    : '<p class="tool-empty">광고 계정을 읽는 중…</p>'}
      ...
      <section class="tool-card">
        ...
        <div class="setup-field"><span>URL 유형<small class="setup-hint">NT · FM 은 랜딩링크 뒤에 이어 붙입니다</small></span>${pills('urltype', URL_TYPES.map(([value, label]) => [label, value]), state.urlType)}</div>
        ${sheetTable()}
      </section>
      ...
      <section class="tool-card">
        <div class="tool-list-head">
          <h3>소재 <small>${picks.length}개</small></h3>
          <div class="tool-list-actions">
            <button type="button" class="tool-add setup-pick"><i data-lucide="image-plus"></i>소재 파일 고르기</button>
            <button type="button" class="tool-clear setup-clear-picks"${picks.length ? '' : ' disabled'}>비우기</button>
          </div>
        </div>
        <input type="file" class="setup-file" accept="image/png,image/jpeg" multiple hidden>
        ...
        <p class="setup-note">프로필 이미지는 틀 광고그룹의 소재에서 물려받습니다${sampleImage() ? '' : ' — 아직 못 가져왔습니다'}.</p>`}
      </section>

      <section class="tool-card">
        <h3>만들기</h3>
        <div class="setup-run">
          <button type="button" class="tool-add setup-make"${work && work.running ? ' disabled' : ''}>
            <i data-lucide="megaphone"></i>${work && work.running ? '만드는 중…' : '카카오에 만들기'}</button>
          <small>캠페인 · 광고그룹을 만들고 소재를 하나씩 올립니다 (카카오가 1초에 하나만 받습니다).</small>
        </div>
        ${issuesBox()}
        ${workBox()}
      </section>
      ...
      <section class="tool-card">
        <div class="tool-list-head">
          <h3>만든 기록 <small>${history.length}건</small></h3>
          <div class="tool-list-actions"><button type="button" class="tool-clear setup-hist-clear"${history.length ? '' : ' disabled'}>기록 비우기</button></div>
        </div>
        ${history.length
    ? `<ul class="setup-hist">${history.map((entry) => `<li><b>${escapeHtml(entry.group)}</b><small>${escapeHtml(entry.kind || '')} · 소재 ${entry.ok}개${entry.bad ? ` (실패 ${entry.bad})` : ''} · 광고그룹 ${escapeHtml(String(entry.groupId || ''))} · ${escapeHtml(entry.at)}</small></li>`).join('')}</ul>`
    : '<p class="tool-empty">아직 만든 것이 없습니다.</p>'}
      </section>`;
    lucide.createIcons();
  };
  ...
  kakaoSetup.addEventListener('change', (event) => {
    const file = event.target.closest('.setup-file');
    if (file) { addFiles(file.files); file.value = ''; return; }
    const field = event.target.closest('[data-field]');
    if (!field || (field.tagName !== 'SELECT' && field.type !== 'date')) return;
    state[field.dataset.field] = field.value;
    save();
  ...
  kakaoSetup.addEventListener('click', (event) => {
    ...
    if (event.target.closest('.setup-hist-clear')) {
      if (!window.confirm(`기록 ${history.length}건을 지울까요?`)) return undefined;
      history = [];
      saveHistory();
      return render();
    }
  ...
  kakaoSetup.addEventListener('keydown', (event) => {
    if (event.key === 'Enter' && event.target.dataset.field === 'event') {
      event.preventDefault();
      lookupSheet();
    }
  });

  kakaoSetup.addEventListener('dragover', (event) => {
    if (!event.target.closest('.setup-drop')) return;
    event.preventDefault();
    event.target.closest('.setup-drop').classList.add('is-over');
  });
  kakaoSetup.addEventListener('dragleave', (event) => {
    const box = event.target.closest('.setup-drop');
    if (box) box.classList.remove('is-over');
  });
  kakaoSetup.addEventListener('drop', (event) => {
    const box = event.target.closest('.setup-drop');
    if (!box) return;
    event.preventDefault();
...
if (utmBuilder) {
  ...
  const waiting = () => entries.filter((entry) => !entry.sent);
  ...
  // 이 화면을 열 때 받아 온다 (페이지가 뜨자마자 받으면 다른 화면을 밀어 낸다).
  openViewOnce(utmBuilder, () => { waitNote = '시트에서 불러오는 중…'; waitTell(); pullWait(false); });
...
if (brandSchedule) {
  ...
  schedNote = '시트에서 불러오는 중…';
  // 이 화면을 열 때 받아 온다. 주소에 일정 번호가 붙어 바로 열리는 경우도
  // openViewOnce 가 그 자리에서 부르므로 링크를 따라 여는 것은 그대로 된다.
  openViewOnce(brandSchedule, () => { schedTell(); pullSchedule().then(() => openLinked()); });
  const commit = ({ redrawPeek = true } = {}) => {
    save();
    renderView();
    if (redrawPeek && openId) renderPeek();
...
if (mediaPerformance) {
  ...
  // 들어가므로 네 단계가 조회 합계와 그대로 맞는다.
  // 상시는 늘 도는 광고(브랜드검색 · 파워링크 같은 것)를 담는 자리다.
  // 다른 단계와 겹쳐 적으면 광고비가 두 번 세지므로, 겹치지 않게 날짜를 잡아 쓴다.
  const PHASES = ['사전', '당일', '사후', '상시'];
  ...
  let crossProd = [];        // 전매체 검색: 체크해 둔 제품 (빈 목록이면 전 제품)
  ...
  /* 이름이 검색어에 걸리나.
     다른 매체는 광고그룹을 '[행사]_타겟팅_매출채널' 로 짓기 때문에 '대괄호 안만' 이 쓸모 있다.
     **네이버 검색광고(브랜드검색)만 그 규칙을 안 쓴다** — 광고그룹은 브랜드 키워드고
     소재 이름은 문구에서 따 오므로 대괄호가 아예 없다. 그러면 perfBracket 이 빈 글자를 주고
     한 줄도 안 걸린다. 이 매체만 결과가 통째로 비던 까닭이 이것이다.
     그래서 여기서는 **이름 전체**로 보고, 소재 이름과 광고그룹 이름을 둘 다 본다. */
  /* 검색어는 **쉼표로 여러 개** 넣을 수 있다 ('kakao, 카카오').
     같은 행사인데 매체마다 한글 · 영문으로 다르게 적어 둔 것을 한 번에 보려는 것이다.
     하나라도 걸리면 그 줄을 가져온다 (모두 걸려야 하는 것이 아니다 — 서로 다른 표기라서).
     쉼표만 가른다. 공백으로도 가르면 행사명 안의 띄어쓰기까지 쪼개져 엉뚱한 줄이 걸린다. */
  const crossWords = (text) => String(text || '').split(',')
    .map((one) => one.trim().toLowerCase())
    .filter(Boolean);

  const crossHit = (key, row, words) => {
    if (!words.length) return true;          // 검색어가 비면 다 본다
    return words.some((one) => (key === 'naverSa'
      ? perfNameHit(row.name, one, false) || perfNameHit(row.campaignName || '', one, false)
      : perfNameHit(row.name, one, crossBracket)));
  };

  /* 제품 체크는 **받아 둔 줄을 다시 거를 뿐** 매체에 다시 묻지 않는다.
     웹앱은 팀이 실행 자리 하나를 나눠 쓰기 때문에, 체크 한 번에 네 매체를 다시 훑으면
     남이 기다린다. 그래서 거르지 않은 줄(raw)을 그대로 쥐고 있다가 여기서 걸러 낸다.
     단계별로 받아 둔 값도 같이 걸러야 화면 위아래 숫자가 어긋나지 않는다. */
  const crossRefilter = () => {
    const words = crossProdWords(crossProd);
    Object.keys(cross).forEach((key) => {
      const one = cross[key];
      if (one && one.raw) one.rows = one.raw.filter((row) => crossProdHit(key, row, words));
    });
    PHASES.forEach((name) => {
      const pack = phaseData[name];
      if (pack && pack.raw) pack.rows = pack.raw.filter((one) => crossProdHit(one.key, one.row, words));
    });
  };
    ...
    return askAccounts(key).then((list) => {
      ...
      return Promise.all(use.map((one) => ask({
        ...
        return { rows: rows, note: trouble.join(' · '), hits: hits, tried: use.length,
          all: all, coverage: coverage, accountName: names.join(' · ') };
      });
    });
  };
    ...
    Promise.all(Object.keys(SOURCES).map((key) => crossAsk(key, period.since, period.until, wanted)
      .then((got) => {
        ...
        cross[key] = {
          ...
          raw: got.rows,          // 제품으로 거르기 전 (체크를 바꿔도 다시 안 묻는다)
          rows: got.rows.filter((row) => crossProdHit(key, row, crossProdWords(crossProd))),
        };
        render();
      })
      .catch((reason) => {
  ...
  const phaseLoad = (only) => {
    ...
    want.forEach((name) => {
      ...
      Promise.all(Object.keys(SOURCES).map((key) => crossAsk(key, span.since, span.until, wanted, crossHits[key])
        ...
        .then((packs) => {
          const all = packs.reduce((into, one) => into.concat(one), []);
          const words = crossProdWords(crossProd);
          phaseData[name] = {
            status: 'ready',
            error: trouble.join(' · '),
            gap: gaps.join(' · '),
            raw: all,             // 제품으로 거르기 전
            rows: all.filter((one) => crossProdHit(one.key, one.row, words)),
          };
          render();
        });
    });
  ...
  /* 제품으로 묶는다 (묶음 하나가 파일 하나).
     ...
     하나를 두는 구조라, 단계까지 파일로 가르면 마지막에 올린 것만 남는다.
     단계마다 그 기간만 따로 받아 온 값이므로, 같은 광고그룹이 사전 · 당일에 다 나와도
     광고비는 각 기간의 값으로 갈라져 있다. */
  /* 같은 제품을 캠페인마다 다르게 적어 둔다 — '더 플렌더(mini)' 와 '더 플렌더(MINI)'.
     **대소문자만 다르면 한 제품으로 묶는다.** 갈라 두면 파일이 둘로 나와 받는 쪽에서
     같은 상품에 두 번 올려야 하고, 한 번 올릴 때마다 앞의 것이 덮인다.
     (월별 예산도 이미 대소문자를 무시해 SKU 를 붙인다 — 두 화면의 기준을 맞추는 것이기도 하다) */
  const productKey = (name) => String(name || '').trim().toLowerCase();

  const productFiles = () => {
    const order = [];
    const box = {};
    /* 보여 줄 이름은 **줄이 가장 많은 표기**를 쓴다 (계정에서 실제로 많이 쓰는 철자다).
       같은 수면 먼저 나온 것을 쓴다 — 볼 때마다 이름이 바뀌지 않게. */
    const room = (product) => {
      const key = productKey(product);
      if (!box[key]) {
        box[key] = { product: product, names: [], searched: [], byPhase: {} };
        order.push(key);
      }
      const pack = box[key];
      const seen = pack.names.filter((one) => one.name === product)[0];
      if (seen) seen.count += 1;
      else pack.names.push({ name: product, count: 1 });
      return pack;
    };
    ...
    const spendOf = (pack) => {
      const rows = PHASES.reduce((into, phase) => into.concat(pack.byPhase[phase] || []), []);
      return totalsOf((rows.length ? rows : pack.searched).map((one) => one.row)).spend;
    };
    // 이름을 정한다 (가장 많이 쓰인 철자)
    order.forEach((key) => {
      const pack = box[key];
      pack.product = pack.names.slice().sort((a, b) => b.count - a.count)[0].name;
    });

    // 광고비가 큰 제품을 앞에 둔다. 제품을 못 바른 줄은 맨 뒤로 보낸다.
    const lost = (key) => box[key].product === NO_PRODUCT;
    order.sort((a, b) => {
      if (lost(a) !== lost(b)) return lost(a) ? 1 : -1;
      return spendOf(box[b]) - spendOf(box[a]);
    });

    return order.map((key) => {
      const pack = box[key];
      const phases = PHASES.filter((phase) => (pack.byPhase[phase] || []).length)
        .map((phase) => ({ name: phase, rows: pack.byPhase[phase] }));
      const rows = phases.length
        ? phases.reduce((into, one) => into.concat(one.rows), [])
        : pack.searched;
      return { product: pack.product, searched: pack.searched, phases: phases, rows: rows,
        // 대소문자만 다른 철자가 섞여 있었으면 무엇을 합쳤는지 알려 준다
        merged: pack.names.length > 1 ? pack.names.map((one) => one.name) : [],
        spend: totalsOf(rows.map((one) => one.row)).spend };
    });
  };

  /* 제품 하나의 파일 속. 세일즈용 파일과 **같은 모양**이라 행사별 결과가 그대로 읽는다.
     다른 점은 이 제품의 줄만 담긴다는 것뿐이다. */
  const productPayload = (file) => {
    const period = currentRange();
    return {
      ...
      // 제품을 골라 놓고 받았으면 그 제품 캠페인만 들어 있다고 적어 둔다
      ...
      phases: file.phases.map((one) => ({
        name: one.name,
        since: phaseSpan[one.name].since,
        until: phaseSpan[one.name].until,
        rows: one.rows.map((each) => salesRow(each.key, each.row)),
      })),
    };
  };

  const safeName = (text) => String(text || '').replace(/[\\/:*?"<>|]/g, '').trim();
  const productName = (file) => `행사별결과_${safeName(crossFor || '검색')}`
    + `_${safeName(file.product) || '제품미상'}.json`;
  const productPick = (want) => productFiles()
    .filter((one) => productKey(one.product) === productKey(want))[0] || null;
  ...
  const salesRow = (key, row) => ({
    ...
    // 번호도 담는다 — 받는 쪽(판매채널 추이)이 이걸로 게재지면 · 연령 · 성별 · 소재를
    // 그때그때 물어본다. 이름만으로는 매체에 되물을 수가 없다.
    id: String(row.id || ''),
  ...
  const salesPayload = () => {
    ...
    return {
      ...
      range: { since: period.since, until: period.until },
      madeAt: new Date().toISOString(),
      note: '카카오모먼트 · 메타 · 구글은 매체가 준 값 그대로, 네이버는 부가세를 뺀 공급가입니다.',
      // 조회 기간 전체 (매체 · 광고그룹 단위)
  ...
  const mixCard = () => {
    ...
    // 단계(사전 · 당일 · 사후 · 상시)를 받아 둔 제품만 내놓는다. 파일에 조회 기간 줄을 담지 않으니
    ...
    return `<div class="perf-mix">
      <div class="perf-mix-head"><b>행사별 결과로 보내기 (제품별)</b>
        <small>제품마다 파일 하나입니다. 매체는 가르지 않고 <b>단계(사전 · 당일 · 사후 · 상시)</b>로만
      ...
      ${files.length ? `<div class="perf-mix-list">
        ${files.map((one) => `<button type="button" class="perf-mix-btn" data-cross="mix"
          ...
          <small>${one.phases.map((each) => `${escapeHtml(each.name)} ${count(each.rows.length)}줄`).join(' · ')}
            · ${money(one.spend)}${one.merged.length
    ? ` · ${escapeHtml(one.merged.join(' + '))} 합침` : ''}</small></span>
        </button>`).join('')}
      </div>` : ''}
  ...
  const crossCard = () => {
    ...
    const lines = Object.keys(SOURCES).map((key) => {
      ...
      const head = `<tr class="perf-row${open ? ' is-open' : ''}" data-cross-row="${key}">
        ...
        : one.status === 'error' ? escapeHtml(one.error || '오류')
          : `광고그룹 ${count(rows.length)}${!rows.length && one.all
        ? ` (매체가 준 ${count(one.all)}개 중 이름이 걸린 것 없음)` : ''}${
        one.accountName ? ` · ${escapeHtml(one.accountName)}` : ''}${
    ...
    return `<div class="tool-card">
      ...
      <div class="perf-filter perf-cross-prod">
        <span class="perf-phase-label">제품</span>
        ${CROSS_PRODUCTS.map((one) => `<label class="perf-check"
          title="캠페인 이름에 ${escapeHtml(one.words.join(' · '))} 가 든 것만 봅니다 (띄어쓰기와 '더' 는 안 봅니다)">
          <input type="checkbox" data-perf="cross-prod" data-prod="${escapeHtml(one.id)}"${crossProd.indexOf(one.id) >= 0 ? ' checked' : ''}>${escapeHtml(one.name)}</label>`).join('')}
        <span class="perf-phase-state">${crossProdText()
      ? `${escapeHtml(crossProdText())} 캠페인 안에서만 찾습니다`
      : '안 고르면 전 제품에서 찾습니다 · 골라 두면 그 캠페인 안에서만 검색어를 봅니다'}</span>
      </div>
      <div class="perf-filter">
        <div class="perf-search">
          <i data-lucide="search"></i>
          <input type="search" data-perf="cross-text" value="${escapeHtml(crossText)}"
            placeholder="광고그룹 이름 · 쉼표로 여러 개 (예: kakao, 카카오)" autocomplete="off">
        </div>
        <label class="perf-check" title="[cj-260901]_none_cj 에서 대괄호 안의 cj-260901 만 봅니다 · 네이버 검색광고는 대괄호를 안 써서 늘 이름 전체로 봅니다">
      ...
      ? `<button type="button" class="tool-copy-all" data-cross="phase-clear"
          title="네 단계의 날짜와 받아 둔 값을 한 번에 지웁니다">
      ...
      ${searched ? `<div class="tool-table-wrap"><table class="tool-table perf-table">
        ${crossHead('매체 · 광고그룹')}
        <tbody>${lines}
          <tr class="perf-row perf-cross-sum"><td class="perf-name"><span><b>합계</b><small>${escapeHtml(crossWords(crossFor).join(' 또는 ') || crossFor)}${crossBracket ? ' (대괄호 안)' : ' (이름 전체)'}${crossProdText() ? ` · ${escapeHtml(crossProdText())}` : ''}
      ...
      ${found ? '' : `<p class="tool-empty">${busy ? '' : `'${escapeHtml(crossFor)}' 가 들어간 광고그룹을 찾지 못했습니다.${crossProdText()
      ? ` (${escapeHtml(crossProdText())} 캠페인 안에서만 봤습니다 — 체크를 풀면 넓게 봅니다)` : ''}`}</p>`}`
    ...
    ask({ action: 'naverSaCostGet' })
      .then((body) => {
        costRows = (body.rows || []).map((one) => ({
          ...
          raw: '',        // 수식으로 적었으면 적은 그대로 (시트에는 센 값만 담긴다)
          bad: false,     // 수식을 못 셌다
          note: one.note || '',
        }));
        costUrl = body.url || '';
    ...
    return `<div class="tool-card">${head}
      <div class="tool-table-wrap"><table class="tool-table perf-cost">
        ...
        <tbody>${rows.map((one, at) => `<tr>
          ...
          <td><input type="text" inputmode="text" class="perf-cost-num${one.bad ? ' is-bad' : ''}"
            data-cost="cost" data-at="${at}" placeholder="3,300,000 · =3300*1000"
            value="${escapeHtml(one.raw || (one.cost ? one.cost.toLocaleString('ko-KR') : ''))}"
            title="${one.raw ? `= ${one.cost.toLocaleString('ko-KR')}원` : ''}"></td>
      ...
      <p class="perf-note">금액은 <b>부가세 포함</b>으로 적습니다 (계약서에 적힌 그 금액).
        표에는 <b>공급가</b>가 들어갑니다 (÷1.1) — 다른 매체와 같은 기준으로 견주려는 것입니다.
        기간은 <b>그 금액을 산 기간</b>이고 조회 기간과 겹친 날수만큼만 들어갑니다 —
        9/1~9/30 에 330만원을 적어 두고 9/1~9/10 을 보면 100만원이 잡힙니다 (330만 ÷ 1.1 × 10/30).
  ...
  mediaPerformance.addEventListener('change', (event) => {
    ...
    if (cost && costRows) {
      ...
      if (cost === 'cost') {
        // '=10*2' 처럼 적어도 받는다. 못 세면 앞서 센 값을 그대로 두고 칸만 빨개진다.
        const typed = event.target.value;
        const found = numValue(typed);
        costRows[at].raw = numFormula(typed) ? typed : '';
        costRows[at].bad = found === null;
        if (found !== null) costRows[at].cost = found;
      } else {
        costRows[at][cost] = event.target.value;
      }
      return;
    }
    ...
    if (field === 'cross-prod') {
      ...
      const at = crossProd.indexOf(id);
      if (at >= 0) crossProd.splice(at, 1); else crossProd.push(id);
      crossRefilter();        // 받아 둔 줄을 다시 거르기만 한다 (매체에 다시 안 묻는다)
      render();
      return;
    }
  ...
  mediaPerformance.addEventListener('click', (event) => {
    ...
    if (mixBtn) {
      const one = productPick(mixBtn.dataset.product);   // 대소문자를 가리지 않고 집는다
...
if (eventResult) {
  ...
  const cellsOf = () => (body ? (body.cells || []).filter((one) => one.month === month) : []);

  // 큰 채널이 위로 (광고비 기준). 달을 바꿔도 기준이 같아 줄이 튀지 않는다.
  const channelRows = () => cellsOf().slice().sort((a, b) => (b.plan - a.plan)
    || String(a.channel).localeCompare(String(b.channel)));

  /* ── 전매체 파일을 판매채널에 붙인다 ────────────────────────────
     광고그룹 이름은 '[행사]_타겟팅_매출채널' 이라 **마지막 토막이 매출채널**이다.
     설정 탭의 행사채널 → 매출채널 표로 되짚어 판매채널과 맞춘다
     (카카오 → kakao · CJ → cj · KOL 라이브 → kolLive …).
  ...
  // 판매채널 → 그 채널로 볼 매출채널 조각들
  const marksOf = (channel) => {
    const out = [plain(channel)];
    // 제휴처럼 한글 이름이 영문 광고그룹으로 도는 것 (찰스엔터 비밀특가 → charlesenter-secret).
    // 서버가 실사용비를 받을 때 쓰는 그 표를 그대로 준다 — 규칙을 한 군데서만 고친다.
    ((body && body.channelAds) || []).forEach((one) => {
  ...
  /* ── 판매채널 하나를 펼쳐 본다 ──────────────────────────────────
     파일에는 광고그룹 줄까지만 들어 있다. 게재지면 · 연령 · 성별 · 소재는
     **누를 때** 매체에 직접 물어본다 (파일에 담아 온 번호로 부른다).
  ...
  const rangeFor = (channel) => {
    ...
    if (!since || !until) {
      ...
      until = until || cell.until || '';
    }
    return { since: since, until: until };
  };

  // 그 채널이 어느 매체 · 계정 · 광고그룹에 걸려 있나 (번호가 있는 줄만)
  const spotsOf = (channel) => {
    const out = [];
    rowsFor(channel).forEach((row) => {
  ...
  const askDetail = (channel, key, again) => {
    ...
    const range = rangeFor(channel);

    // 왜 못 묻는지 먼저 말해 준다. 예전에는 그냥 빈 표가 나와 까닭을 알 수 없었다.
    const stop = (why) => {
      got[slot] = { status: 'ready', rows: [], notes: [why], range: range };
      render();
    };
    if (!spots.length) {
    ...
    got[slot] = { status: 'loading', rows: [], notes: [], range: range };
    render();

    const notes = [];
    const noSplit = [];
    const mine = spots.filter((one) => {
      if (key !== 'creative' && SPLITS.indexOf(one.source) < 0) {
        if (noSplit.indexOf(one.sourceName) < 0) noSplit.push(one.sourceName);
        return false;
      }
      return true;
    });
    if (noSplit.length) notes.push(`${noSplit.join(' · ')} 는 쪼개 보기를 주지 않습니다.`);
    if (!mine.length) {
      got[slot] = { status: 'ready', rows: [], notes: notes, range: range };
      render();
      return;
    }

    Promise.all(mine.map((one) => askSheet(key === 'creative'
      ? { action: `${one.source}Creatives`, account: one.account, adset: one.adset,
        since: range.since, until: range.until }
      : { action: `${one.source}Breakdown`, account: one.account, adset: one.adset,
        breakdown: key, since: range.since, until: range.until })
      .then((found) => ((key === 'creative' ? found.creatives : found.rows) || [])
        .map((row) => ({ ...row, sourceName: one.sourceName })))
      .catch((reason) => {
        notes.push(`${one.sourceName} · ${one.name} — ${reason.message}`);
        return [];
      }))).then((packs) => {
      const rows = packs.reduce((into, each) => into.concat(each), []);
      got[slot] = { status: 'ready', rows: rows, notes: notes, range: range };
      render();
    });
  };

  // 같은 이름끼리 더한다 (매체가 여럿이면 같은 '피드' 가 여러 줄로 온다)
  const mergeRows = (rows) => {
    const by = {};
    rows.forEach((row) => {
      const name = String(row.name || '(알 수 없음)') + (row.series ? ` · ${row.series}` : '');
      if (!by[name]) by[name] = { name: name, spend: 0, impressions: 0, clicks: 0, linkClicks: 0, purchase: 0, results: 0, revenue: 0 };
      const into = by[name];
      into.spend += Number(row.spend) || 0;
      into.impressions += Number(row.impressions) || 0;
      into.clicks += Number(row.clicks) || 0;
      into.linkClicks += Number(row.linkClicks) || 0;
      into.purchase += Number(row.purchase) || 0;
      into.results += Number(row.results) || 0;
      into.revenue += Number(row.revenue) || 0;
    });
    return Object.keys(by).map((name) => by[name]).sort((a, b) => b.spend - a.spend);
  };

  const splitTable = (rows) => {
    const merged = mergeRows(rows);
    if (!merged.length) return '<p class="perf-note">받아 온 줄이 없습니다.</p>';
    const chart = vizBars(merged.map((one) => ({ name: one.name, value: one.spend })),
      (value) => won(value), '광고비');
    return `${chart}
      <div class="tool-table-wrap"><table class="tool-table tr-split">
        ...
        <tbody>${merged.map((one) => `<tr>
          ...
          <td class="perf-num">${num(one.impressions)}</td>
  ...
  const creativeCards = (rows) => {
    const shown = rows.slice().sort((a, b) => (Number(b.spend) || 0) - (Number(a.spend) || 0)).slice(0, 24);
    if (!shown.length) return '<p class="perf-note">받아 온 소재가 없습니다.</p>';
    return `<div class="tr-cuts">${shown.map((one) => `<figure class="tr-cut">
      ...
      <figcaption>
        ...
        <em>${money(one.spend)} · 전환 ${one.purchase ? num(one.purchase) : 0} · ROAS ${one.spend ? perfRoas(ratio(one.revenue, one.spend) || 0) : '—'}</em>
      </figcaption>
    </figure>`).join('')}</div>`;
  };
  ...
  const render = () => {
    if (status === 'loading') {
      eventResult.innerHTML = '<div class="tool-head"><h2>판매채널 추이</h2></div>'
        + '<div class="tool-card"><p class="perf-note">시트에서 읽는 중…</p></div>';
      return;
    }
...
//   ① 총 예산    **적는 칸이 없다** — ④ 에서 SKU 마다 적은 예산의 합이다.
//   ② 요약       구분마다 예정(노랑) · 사용(파랑) 을 나란히 그린다. 아래에서 올라온 값이다.
//   ③ 카테고리별  더 플렌더 · 생활가전. **적는 칸이 없다** — ④ 의 SKU 와 고정비를 묶은 합이다.
//   ④ 프로모션    여기만 손으로 적는다. SKU 마다 **예산**을 적고, 그 아래 판매채널 줄에
//                목표수량 × 목표 CPS = 광고비로 그 예산을 나눠 쓴다.
//                그래서 SKU 에는 숫자가 둘이다 — 배정한 **예산**과 줄로 짠 **계획**.
//                둘이 어긋나면 (계획이 예산을 넘거나 남으면) 그 자리에서 바로 보인다.
//
// 화면은 **보기**와 **고치기** 두 모습이다. 평소에는 값만 보이고(입력칸도 단추도 없다),
// [수정하기] 를 눌러야 적는 칸이 열린다. 다 적고 [저장하기] 를 누르면 다시 보기로 돌아간다.
// 짜는 사람은 한둘인데 보는 사람은 여럿이라, 기본을 보기로 두는 편이 안전하다.
//
// **실사용비는 모두 손으로 적는다.** 앞서는 매체를 하나씩 불러 채워 주는 길이 있었는데,
// 몇 분이 걸리는 데다 Apps Script 웹앱은 팀 전체가 실행 줄 하나를 같이 써서 그동안
// 다른 사람 화면까지 멈췄다 (빈 요청이 32초까지 밀렸다). 그래서 걷어 냈다.
// 고정비 쪽은 나중에 네이버 검색광고 API 로 따로 채울 예정이다 — 그 매체 하나만 부르므로
// 전부를 훑던 옛 방식과 다르다.
...
if (budgetPlanView) {
  ...
  // 카테고리와 그 안의 상세 SKU. 여기 적힌 것만 이 달의 예산에 넣을 수 있다.
  // 이름은 캠페인명에 쓰는 제품 이름과 같게 둔다 (사람이 오갈 때 헷갈리지 않게).
  const SKU_TREE = [
    ['더 플렌더', ['더 플렌더mini', '더 플렌더MAX', '더 플렌더PRO']],
    ['생활가전', ['더 에어드라이', '더 시프트']],
  ];
  const CATEGORIES = SKU_TREE.map(([name]) => name);
  const ALL_SKUS = SKU_TREE.reduce((all, [, list]) => all.concat(list), []);
  ...
  /* 캠페인에서 발라낸 제품 이름이 갈 자리를 정한다.
     ...
     둘 다 아니면 null — 못 붙였다고 알려 주고 사람이 손으로 옮긴다. */
  const spotOf = (product) => {
    const want = nameKey(product);
    if (!want) return null;
    const sku = ALL_SKUS.find((one) => nameKey(one) === want);
    if (sku) return { category: categoryOf(sku), sku: sku };
    const category = CATEGORIES.find((one) => nameKey(one) === want);
    return category ? { category: category, sku: '' } : null;
  };
  const categoryOf = (sku) => {
    const found = SKU_TREE.find(([, list]) => list.indexOf(sku) >= 0);
    return found ? found[0] : CATEGORIES[0];
  };

  /* 고정비 — 프로모션과 상관없이 달마다 나가는 돈이다 (검색광고 정액 · 솔루션 이용료 같은 것).
     프로모션 표와 달리 목표수량 · CPS 로 셀 것이 없어, 쓸 돈(사용예정)과 쓴 돈(실사용비)을
     그냥 적는다. 어느 제품 몫인지 가려 두려고 카테고리 · 상세 SKU 를 함께 고른다
  ...
  const CHANNELS = [
    ...
    ['제휴 (실사용비 자동)', ['찰스엔터 비밀특가', '고기남자 어필리에잇', '아가리어터 비밀특가']],
  ];
  // 구분 · 파트 — 그 줄을 누가 어디로 돌리는지. 두 칸을 나란히 둔다.
  const GROUPS = ['세일즈', '마케팅팀', '퍼포프젝'];
  const GROUP_LIST = 'bg-groups';
  const PARTS = ['온라인', '오프라인', '마케팅팀', '퍼포프젝'];

  // 그 줄에서 돌리는 광고. 여러 개를 함께 켤 수 있다 (브랜드검색 + CRM 처럼).
  // 표 칸 안이라 여러 줄 고르개(select multiple)는 너무 높다 — 눌러 켜는 조각으로 둔다.
  const AD_KINDS = ['브랜드검색', '페이드', 'CRM'];
  ...
  let plan = { skus: [], rows: [] };
  let saved = { at: '', by: '' };
  let editing = false;           // 고치는 중인가 (평소에는 보기)
  let fresh = false;             // 시트에서 온 값인가 (아니면 담아 둔 옛 판)
  let fixedPulling = false;      // 고정비 사용액을 네이버에서 받는 중인가
  let pushing = false;           // 받은 값을 구글시트에 올리는 중인가
  let fixedNote = '';            // 받아 온 결과 한 줄
  let status = 'idle';
  let error = '';
  ...
  /* 브랜드검색비는 **고정비에 이미 들어 있다** (고정비 항목의 '브랜드검색').
     프로모션에도 적어 두는 것은 '이 프로모션에 브검이 얼마나 붙었나' 를 곁에서 보려는
     참고용이다. 그래서 어떤 합계에도 더하지 않는다 — 더하면 같은 돈을 두 번 세게 된다.
     체크를 켜면 프로모션 표에서만 얹어 보여 준다 (곁말로 견주려는 것이다).
     보는 사람 것이라 시트에 담지 않는다. */
  let withBrand = false;
  const rowBrand = (row) => Number(row.brand) || 0;
  // 프로모션 표에 적어 보일 금액. 체크를 켜야만 브검비가 얹힌다.
  const rowShown = (row) => rowCost(row) + (withBrand ? rowBrand(row) : 0);
  // 브검비 칸을 보여 줄까 — 고치는 중이면 늘, 보기일 때는 적어 둔 줄이 있을 때만.
  const hasBrand = (name) => editing || rowsOf(name).some((row) => (Number(row.brand) || 0) > 0);

  // 광고비는 목표수량 × 목표 CPS 로 센다. 그 줄에 금액을 직접 적어 두었으면 그 값이 이긴다
  // (정액 상품 · 협찬비처럼 수량으로 안 떨어지는 줄이 있다). 브검비는 어느 쪽이든 더한다.
  const rowManual = (row) => (Number(row.cost) || 0) > 0;
  const rowCost = (row) => {
    const base = rowManual(row) ? Number(row.cost) : (Number(row.goal) || 0) * (Number(row.cps) || 0);
    return base;   // 브검비는 안 더한다 (고정비에 이미 있다)
  };
  const rowsOf = (name) => plan.rows.filter((row) => row.sku === name);

  // 한 SKU 의 숫자 둘.
  //   budget  사람이 그 SKU 에 배정한 예산 (여기만 손으로 적는다)
  //   planned 아래 프로모션 줄로 짠 금액 (목표수량 × 목표 CPS 의 합)
  const skuBudget = (name) => Number((skuOf(name) || {}).budget) || 0;

  const skuSum = (name) => {
    const rows = rowsOf(name);
    const planned = rows.reduce((sum, row) => sum + rowCost(row), 0);   // 브검 없는 진짜 금액
    const brand = rows.reduce((sum, row) => sum + rowBrand(row), 0);
    const shown = rows.reduce((sum, row) => sum + rowShown(row), 0);    // 지금 보는 기준
    const goal = rows.reduce((sum, row) => sum + (Number(row.goal) || 0), 0);
    // 완료 = 실사용비를 적은 줄. 돈이 나갔으면 그 프로모션은 끝난 것으로 본다.
    const done = rows.filter((row) => rowUsed(row) > 0).length;
    const used = rows.reduce((sum, row) => sum + rowUsed(row), 0);
    return {
      rows: rows.length,
      done: done,
      used: used,
      budget: skuBudget(name),
      planned: planned,
      brand: brand,
      shown: shown,
      goal: goal,
      cps: goal > 0 ? shown / goal : 0,
    };
  };

  // 실사용비 — 줄마다 손으로 적는다.
  const rowUsed = (row) => Number(row.used) || 0;
  const skuUsed = (name) => rowsOf(name).reduce((sum, row) => sum + rowUsed(row), 0);

  // 맨 위 총 예산은 SKU 예산의 합이다 (더는 위에서 따로 적지 않는다).
  const budgetAll = () => plan.skus.reduce((sum, one) => sum + skuBudget(one.name), 0);
  const plannedAll = () => plan.skus.reduce((sum, one) => sum + skuSum(one.name).planned, 0);

  // SKU 가 어느 카테고리인지는 위 표에서 정한다 (담아 둔 값은 옛 데이터를 위해 남겨 둔다)
  const catOf = (sku) => categoryOf(sku.name) || sku.category || CATEGORIES[0];

  const categorySum = (name) => {
    const mine = plan.skus.filter((one) => catOf(one) === name);
    const budget = mine.reduce((sum, one) => sum + skuBudget(one.name), 0);
    const planned = mine.reduce((sum, one) => sum + skuSum(one.name).planned, 0);
    const goal = mine.reduce((sum, one) => sum + skuSum(one.name).goal, 0);
    // 그 카테고리에 붙은 고정비도 예정 · 사용에 함께 넣는다
    const fixed = fixedSum(name);
    const used = mine.reduce((sum, one) => sum + skuUsed(one.name), 0) + fixed.used;
    return { skus: mine, budget, planned: planned + fixed.plan, goal, used };
  };

  const fixedRows = () => (Array.isArray(plan.fixed) ? plan.fixed : []);
  const fixedSum = (category) => fixedRows().reduce((all, row) => (
    category !== undefined && String(row.category || '') !== category ? all : {
      plan: all.plan + (Number(row.plan) || 0),
      used: all.used + (Number(row.used) || 0),
    }), { plan: 0, used: 0 });
  ...
  // ① 총 예산 — **적는 칸이 없다.** SKU 마다 적은 예산을 더한 값이다.
  const heroCard = () => {
    const sum = budgetAll();
    const fixed = fixedSum();
    const made = plannedAll() + fixed.plan;               // 예정 = 프로모션 계획 + 고정비 사용예정
    // 사용 = 프로모션 줄에 적은 실사용비 + 고정비 실사용비
    const used = plan.skus.reduce((all, one) => all + skuUsed(one.name), 0) + fixed.used;
    const known = used > 0;
    const left = sum - used;
    return `<div class="tool-card bg-hero">
      <div class="bg-hero-head">
        ...
        <small class="bg-hero-note">예산은 SKU ${num(plan.skus.length)}개의 합 ·
          예정 · 사용에는 고정비가 함께 들어갑니다</small>
      </div>
      <div class="bg-stats">
        ${statBox('총 예산', eok(sum), sum ? won(sum) : (editing ? 'SKU 마다 예산을 적어 주세요' : '아직 안 적었습니다'))}
        ${statBox('총 예정', eok(made), made ? won(made) : '아직 안 짰습니다',
    sum && made > sum ? 'is-over' : '')}
        ${statBox('총 사용', known ? eok(used) : '—', known ? won(used) : '사용액을 아직 안 받았습니다')}
        ${statBox('총 잔여', known ? eok(left) : '—', known ? won(left) : '', known && left < 0 ? 'is-over' : '')}
        ${statBox('사용(%)', known ? pct(used, sum) : '—', '사용 ÷ 예산')}
        ${statBox('잔여(%)', known ? pct(left, sum) : '—', '예산 − 사용')}
      </div>
      <p class="bg-line bg-hero-line${sum && made > sum ? ' is-over' : ''}">${!sum || !made ? ''
    : `프로모션 · 고정비로 짜 둔 금액은 <b>${eok(made)}</b> 입니다 — ${made > sum
      ? `SKU 예산 합보다 <b>${eok(made - sum)}</b> 많습니다.`
      : made === sum ? '예산과 딱 맞습니다.' : `아직 <b>${eok(sum - made)}</b> 안 짰습니다.`}`}</p>
    </div>`;
  };

  /* ② 요약 — 구분마다 짠 돈(예정)과 쓴 돈(사용)을 나란히 그린다.
     예정은 그 구분에 달린 프로모션 줄의 광고비 합이라 바로 나온다.
     사용은 매체가 **제품(SKU)** 기준으로만 주기 때문에 구분으로 곧장 갈리지 않는다.
     그래서 그 SKU 의 사용액을 **그 안의 계획 비중대로** 구분에 나눠 붙인다 —
     한 SKU 를 세일즈와 퍼포프젝이 나눠 돌렸으면 계획한 만큼 나눠 가진다.
     고정비는 실사용비를 직접 적으므로 나눌 것 없이 그대로 한 줄을 차지한다. */
  const NO_GROUP = '(구분 없음)';
  const FIXED_GROUP = '고정비';

  const groupSums = () => {
    const out = {};
    const put = (name, key, value) => {
      if (!out[name]) out[name] = { plan: 0, used: 0 };
      out[name][key] += value;
    };
    const nameOf = (row) => String(row.group || '').trim() || NO_GROUP;

    plan.rows.forEach((row) => put(nameOf(row), 'plan', rowCost(row)));
    plan.rows.forEach((row) => put(nameOf(row), 'used', rowUsed(row)));

    const fixed = fixedSum();
    if (fixed.plan || fixed.used) {
      out[FIXED_GROUP] = { plan: fixed.plan, used: fixed.used };
    }

    // 정해 둔 구분을 앞에, 직접 적은 것을 뒤에, 고정비를 맨 끝에 둔다
    const order = GROUPS.concat(Object.keys(out).filter((one) =>
      GROUPS.indexOf(one) < 0 && one !== FIXED_GROUP && one !== NO_GROUP))
      .concat([NO_GROUP, FIXED_GROUP]);
    return order.filter((one) => out[one]).map((one) => ({
      name: one, plan: out[one].plan, used: out[one].used,
    }));
  };

  /* 고리 하나. 활은 원 하나에 stroke-dasharray 로 그린다 —
     둘레를 활 길이만큼 잘라 두고 나머지를 비워 두면 그만큼만 그려진다.
     12시에서 시작하도록 -90도 돌린다. */
  const RING = 54;                                   // 반지름
  const RING_ROUND = 2 * Math.PI * RING;             // 둘레
  const donut = (one) => {
    const part = one.plan > 0 ? one.used / one.plan : 0;
    const over = part > 1;
    const size = Math.min(1, part) * RING_ROUND;      // 넘겨도 한 바퀴까지만 그린다
    const rate = one.plan > 0 ? pct(one.used, one.plan) : '—';
    return `<figure class="bg-donut${over ? ' is-over' : ''}">
      <div class="bg-ringbox">
        <svg viewBox="0 0 120 120" role="img"
          aria-label="${escape(one.name)} 예정 ${escape(won(one.plan))} 중 ${escape(won(one.used))} 사용">
          <circle class="bg-arc-none" cx="60" cy="60" r="${RING}"></circle>
          <g transform="rotate(-90 60 60)">
            ${one.plan > 0 ? `<circle class="bg-arc is-plan" cx="60" cy="60" r="${RING}"><title>예정 ${escape(won(one.plan))}</title></circle>` : ''}
            ${size > 0 ? `<circle class="bg-arc is-used" cx="60" cy="60" r="${RING}"
              stroke-dasharray="${size.toFixed(2)} ${(RING_ROUND - size).toFixed(2)}"><title>사용 ${escape(won(one.used))} (${rate})</title></circle>` : ''}
          </g>
        </svg>
        <figcaption><b>${escape(one.name)}</b><small>${rate}</small></figcaption>
      </div>
      <div class="bg-ringfoot">
        <span title="예정 ${escape(won(one.plan))}"><i>예정</i><b>${eok(one.plan)}</b></span>
        <span title="사용 ${escape(won(one.used))}"><i>사용</i><em>${one.used ? eok(one.used) : '—'}</em></span>
      </div>
    </figure>`;
  };

  const summaryCard = () => {
    const rows = groupSums();
    return `<div class="tool-card bg-summary">
      <div class="tool-list-head"><h3>요약
        <small><i class="bg-key is-plan"></i>예정 &nbsp;<i class="bg-key is-used"></i>사용
          · 고리 하나가 구분 하나입니다 — 가운데 숫자는 예정 대비 사용률</small></h3></div>
      ${rows.length ? `<div class="bg-rings">${rows.map(donut).join('')}</div>`
    : '<p class="perf-note">아직 짠 것이 없습니다. 아래 프로모션 · 고정비에 적으면 여기에 모입니다.</p>'}
      <p class="perf-note">예정은 프로모션 줄의 광고비, 사용은 그 줄에 적은 <b>실사용비</b>입니다.
        고정비도 적어 둔 사용예정 · 실사용비를 그대로 씁니다.</p>
    </div>`;
  };
  ...
  const categoryCards = () => `<div class="bg-grid">${CATEGORIES.map((name) => {
    ...
    return `<div class="tool-card bg-card">
      ...
      <div class="bg-stats">
        ...
        ${statBox('총 예정', eok(one.planned), '프로모션 + 고정비',
    one.budget && one.planned > one.budget ? 'is-over' : '')}
        ${statBox('총 사용', one.used === null ? '—' : eok(one.used),
    one.used === null ? '아직 안 받았습니다' : '실제 광고비')}
        ${statBox('사용(%)', one.used === null ? '—' : pct(one.used, one.budget), '사용 ÷ 예산')}
        ${statBox('잔여(%)', left === null ? '—' : pct(left, one.budget), '예산 − 사용',
    left !== null && left < 0 ? 'is-over' : '')}
      </div>
  ...
  // ④ 프로모션 — 여기만 손으로 적는다
  /* 판매채널은 **고르되 적어도 된다.** 늘 쓰는 곳은 목록으로 깔아 두지만, 새로 생긴 채널이나
     한 번 쓰고 마는 곳까지 목록에 넣어 둘 수는 없다. 그래서 고르개(select) 가 아니라
     목록이 붙은 적는 칸(datalist) 을 쓴다 — 눌러 고를 수도, 그냥 칠 수도 있다. */
  const CHANNEL_LIST = 'bg-channels';
  const channelNames = () => {
    const out = [];
    CHANNELS.forEach(([, list]) => list.forEach((one) => { if (out.indexOf(one) < 0) out.push(one); }));
    // 이 달에 이미 적어 둔 채널도 목록에 넣는다 (직접 적은 곳을 다음 줄에서 또 치지 않게)
    plan.rows.forEach((row) => {
      const one = String(row.channel || '').trim();
      if (one && out.indexOf(one) < 0) out.push(one);
    });
    return out;
  };
  const channelDatalist = () => `<datalist id="${CHANNEL_LIST}">${channelNames()
    .map((one) => `<option value="${escape(one)}"></option>`).join('')}</datalist>`;

  // 구분도 같다 — 정해 둔 세 가지에 더해, 이 달에 이미 적어 둔 것을 목록에 얹는다
  const groupNames = () => {
    const out = GROUPS.slice();
    plan.rows.forEach((row) => {
      const one = String(row.group || '').trim();
      if (one && out.indexOf(one) < 0) out.push(one);
    });
    return out;
  };
  ...
  /* 프로모션 줄의 차례. 담아 둔 배열 차례가 곧 보이는 차례다.
     [날짜순] 을 누르면 광고기간 시작일로 줄을 세우고, 그 뒤에 손으로 끌어 고칠 수 있다.
     늘 날짜순으로 강제하지 않는 까닭: 같은 날 여러 채널을 돌릴 때 어느 것을 위에 둘지는
     사람이 정하고 싶어 하고, 날짜를 아직 안 적은 줄도 자리를 지켜야 하기 때문이다. */
  const sortRows = (name) => {
    const at = [];
    plan.rows.forEach((row, i) => { if (row.sku === name) at.push(i); });
    const mine = at.map((i) => plan.rows[i]);
    // 날짜를 안 적은 줄은 뒤로 보낸다 (앞에 두면 짜다 만 줄이 표를 가린다)
    mine.sort((a, b) => {
      const one = a.since || '';
      const two = b.since || '';
  ...
  const eventRow = (row) => {
    ...
    // 보기일 때는 값만, 고치는 중일 때만 칸을 연다
    const paid = Number(row.used) || 0;
    if (!editing) {
      return `<tr>
    ...
    : dash}</td>
        ${hasBrand(row.sku) ? `<td class="perf-num bg-aside">${row.brand ? won(row.brand) : dash}</td>` : ''}
        <td class="perf-num bg-cost"><span${rowManual(row) ? ' class="is-manual" title="수기로 적은 금액입니다"' : ''}>${cost ? won(cost) : dash}</span></td>
        <td class="perf-num bg-used">${paid ? won(paid) : dash}</td>
      </tr>`;
    }
    return `<tr data-line="${id}">
      ...
      <td><input list="${GROUP_LIST}" class="bg-group" data-bg="group" data-row="${id}"
        value="${escape(row.group || '')}" placeholder="고르거나 직접"></td>
      <td><select data-bg="kind" data-row="${id}">${PARTS.map((one) =>
    `<option${one === row.kind ? ' selected' : ''}>${escape(one)}</option>`).join('')}</select></td>
      ...
      <td class="perf-num"><input type="text" class="bg-num" data-bg="cps" data-row="${id}" value="${escape(row.cps ? commaNum(row.cps) : '')}" placeholder="0"></td>
      <td class="bg-kinds">${AD_KINDS.map((one) => `<button type="button"
        class="bg-kind${mediaOf(row).indexOf(one) >= 0 ? ' is-on' : ''}"
        data-bg="media" data-row="${id}" data-media="${escape(one)}">${escape(one)}</button>`).join('')}</td>
      ${hasBrand(row.sku) ? `<td class="perf-num"><input type="text" class="bg-num bg-aside" data-bg="brand" data-row="${id}" value="${escape(row.brand ? commaNum(row.brand) : '')}" placeholder="0"></td>` : ''}
      <td class="perf-num bg-cost"><input type="text" class="bg-num${rowManual(row) ? ' is-manual' : ''}"
  ...
  const skuPanel = (sku) => {
    ...
    const all = budgetAll();
    const share = all > 0 ? sum.budget / all : 0;
      ...
      <div class="bg-sku-head">
        ...
        ${editing ? `<label class="bg-sku-budget">예산
          <input type="text" inputmode="numeric" data-bg="skuBudget" data-sku="${name}"
            ...
            placeholder="0 · =35000*1200"></label>` : ''}
        ${editing ? `<button type="button" class="bg-drop-sku" data-bg="dropSku" data-sku="${name}"
          title="이 SKU 를 이 달에서 뺍니다">SKU 빼기</button>` : ''}
      </div>
      <div class="bg-stats">
        ${statBox('총 갯수', `${num(sum.rows)}건`, '프로모션 줄 수')}
        ${statBox('완료갯수', `${num(sum.done)}건`,
    sum.rows ? `실사용비를 적은 줄 · ${pct(sum.done, sum.rows)}` : '아직 없습니다')}
        ${statBox('믹스예산', eok(sum.shown), sum.shown
    ? `프로모션 광고비 합 · 브검 ${withBrand ? '포함' : '미포함'}` : '프로모션 줄이 없습니다',
    sum.budget && sum.shown > sum.budget ? 'is-over' : '')}
        ${statBox('실사용예산', sum.used ? eok(sum.used) : '—',
    sum.used ? won(sum.used) : '아직 안 적었습니다')}
        ${statBox('차액', sum.shown || sum.used ? eok(sum.shown - sum.used) : '—',
    '믹스예산 − 실사용예산', sum.shown - sum.used < 0 ? 'is-over' : '')}
      </div>
      <span class="bg-weight"><i style="width:${Math.min(100, share * 100).toFixed(1)}%"></i></span>
      <p class="bg-line">예산 <b>${eok(sum.budget)}</b>${sum.budget
    ? ` · 예산 대비 잔여 <b>${eok(sum.budget - used)}</b> (${pct(used, sum.budget)} 사용)` : ' — 아직 안 적었습니다'}
    · 총 목표수량 <b>${sum.goal ? `${num(sum.goal)}대` : '—'}</b>
    · 평균 CPS <b>${sum.cps ? won(sum.cps) : '—'}</b></p>
      <div class="tool-table-wrap"><table class="tool-table bg-table">
        <thead><tr>
          ...
          <th class="perf-num">실사용비</th>${editing ? '<th></th>' : ''}
        </tr></thead>
        <tbody>${rows.length ? rows.map(eventRow).join('')
    : `<tr><td colspan="${wide}" class="bg-none">${editing
      ? '아직 프로모션이 없습니다. 아래에서 줄을 더해 주세요.' : '적어 둔 프로모션이 없습니다.'}</td></tr>`}</tbody>
      </table></div>
      ${editing ? `<div class="bg-row-tools">
        <button type="button" class="tool-add bg-add" data-bg="add" data-sku="${name}">
          <i data-lucide="plus"></i>프로모션 줄 추가</button>
        ${rows.length > 1 ? `<button type="button" class="tool-copy-all" data-bg="sortRows" data-sku="${name}">
          <i data-lucide="arrow-down-up"></i>광고기간 날짜순</button>
        <small>손잡이(⠿)를 끌면 차례를 손으로 바꿀 수 있습니다.</small>` : ''}
      </div>` : ''}
    </div>`;
  };

  // 카테고리로 가르지 않는다 — 넣어 둔 SKU 를 한 판에 다 편다 (더 플렌더와 더 시프트가
  // 나란히 보여야 어디에 얼마를 실었는지 한눈에 견줄 수 있다). 카테고리 합은 ③ 에서 본다.
  // 넣는 고르개만 카테고리로 묶어 둔다 (목록이 길어져도 찾기 쉽게).
  const events = () => {
    const left = SKU_TREE
      .map(([name, list]) => [name, list.filter((one) => !skuOf(one))])
      .filter(([, list]) => list.length);
    const many = left.reduce((sum, [, list]) => sum + list.length, 0);
    return `<div class="tool-card bg-pick">
      <div class="bg-pick-add">
        <label class="bg-check" title="켜면 이 표에만 브검비를 얹어 봅니다. 어떤 합계에도 안 들어갑니다.">
          <input type="checkbox" data-bg="withBrand"${withBrand ? ' checked' : ''}>브검비 얹어 보기</label>
        <small class="bg-both-note">브랜드검색비는 <b>고정비에 이미 들어 있어</b> 어떤 합계에도 안 더합니다.
          ${withBrand ? '지금은 참고로 <b>얹어</b> 보고 있습니다.' : '켜면 이 표에만 참고로 얹어 보입니다.'}</small>
        ${editing ? `<label class="bg-cat"><b>상세 SKU</b>
          <select data-bg="addSku"${many ? '' : ' disabled'}>
            <option value="" selected>${many ? '넣을 SKU 를 고르세요' : '고를 SKU 를 다 넣었습니다'}</option>
    ...
    : '위 <b>수정하기</b> 를 누르고 <b>상세 SKU</b> 에서 상품을 고르면 표가 열립니다.'}</li></ul></div>`}`;
  };

  /* 고정비 표. 한 줄이 항목 하나다.
       사용예정 · 실사용비만 적고 잔여비 · 사용(%) · 잔여(%) 는 그 자리에서 센다. */
  const fixedLine = (row) => {
    const want = Number(row.plan) || 0;
    const paid = Number(row.used) || 0;
    const spare = want - paid;
    const id = escape(row.id);
    const pick = (key, list, value, empty) => `<select data-bg="${key}" data-row="${id}">
      <option value=""${value ? '' : ' selected'}>${escape(empty)}</option>
      ${list.map((one) => `<option${one === value ? ' selected' : ''}>${escape(one)}</option>`).join('')}
    </select>`;
    // 카테고리를 고르면 그 안의 SKU 만 고르게 한다 (안 골랐으면 전부)
    const skus = row.category
      ? (SKU_TREE.find(([name]) => name === row.category) || ['', []])[1]
      : SKU_TREE.reduce((all, [, list]) => all.concat(list), []);

    if (!editing) {
      return `<tr>
        <td>${escape(row.category || '') || '<span class="tool-blank">공통</span>'}</td>
        <td>${escape(row.sku || '') || '<span class="tool-blank">공통</span>'}</td>
        <td class="perf-name"><span>${escape(row.item || '') || dash}</span></td>
        <td class="perf-num">${want ? won(want) : dash}</td>
        <td class="perf-num">${paid ? won(paid) : dash}</td>
        <td class="perf-num${spare < 0 ? ' is-minus' : ''}">${want ? won(spare) : dash}</td>
        <td class="perf-num">${pct(paid, want)}</td>
    ...
    return `<tr>
      ...
      <td class="perf-num"><input type="text" class="bg-num" data-bg="fixedUsed" data-row="${id}"
        value="${escape(paid ? commaNum(paid) : '')}" placeholder="0"></td>
      <td class="perf-num${spare < 0 ? ' is-minus' : ''}">${want ? won(spare) : dash}</td>
      <td class="perf-num">${pct(paid, want)}</td>
      <td class="perf-num">${want ? pct(spare, want) : '—'}</td>
      <td class="bg-kill"><button type="button" data-bg="fixedDrop" data-row="${id}"
        title="이 줄을 지웁니다"><i data-lucide="x"></i></button></td>
    </tr>`;
  };

  /* 실사용비를 언제 받았는지. 숫자만 있으면 '이게 오늘 것인가' 를 알 수 없다.
     특히 애드부스트는 GFA 라 실시간이 아니다 — PC 에서 스크래퍼를 돌려야 성과 시트에 쌓인다.
     그래서 **받은 시각** 과 **성과 시트에 마지막으로 쌓인 시각** 을 따로 적는다.
     둘이 많이 벌어져 있으면 스크래퍼를 안 돌린 것이다. */
  const clockText = (iso) => {
    const when = new Date(iso);
    return Number.isFinite(when.getTime()) ? when.toLocaleString('ko-KR') : '';
  };
  const fixedStamp = () => {
    const mark = plan.fixedAt;
    const got = mark && clockText(mark.at);
    if (!got) return '';
    const gfa = mark.gfa ? clockText(mark.gfa) : '';
    return ` · 네이버에서 받음 <b>${escape(got)}</b>${gfa
      ? ` · 애드부스트 성과 시트 최종 적재 <b>${escape(gfa)}</b>` : ''}`;
  };
  ...
  const fixedCard = () => {
    ...
    <div class="tool-card">
      <div class="bg-stats">
        ${statBox('사용예정', eok(sum.plan), sum.plan ? won(sum.plan) : (editing ? '적어 주세요' : '아직 없습니다'))}
        ${statBox('실사용비', eok(sum.used), sum.used ? won(sum.used) : '')}
        ${statBox('잔여비', sum.plan ? eok(spare) : '—', '사용예정 − 실사용비', spare < 0 ? 'is-over' : '')}
      ...
      <div class="tool-table-wrap"><table class="tool-table bg-table bg-fixed">
        ...
        <tbody>${rows.length ? rows.map(fixedLine).join('')
    : `<tr><td colspan="${wide}" class="bg-none">${editing
  ...
  /* 이 달의 파워링크 · 쇼핑검색 · 애드부스트 광고비를 받아 고정비에 채운다.
     앞의 둘은 네이버 검색광고 API 에서, 애드부스트는 GFA 라 API 가 없어
     스크래퍼가 쌓아 둔 네이버성과 시트에서 온다 (서버가 한 번에 묶어 준다).
     브랜드검색은 정액(CPT) 이라 광고비가 늘 0 으로 와서 넣지 않는다 — 계약서 보고 손으로 적는다.
     한 번 물으면 몇 초면 끝난다 (매체를 전부 훑던 옛 길과 다르다). */
  const fixedPull = (push) => {
    const last = new Date(Number(month.slice(0, 4)), Number(month.slice(5, 7)), 0).getDate();
    fixedPulling = true;
    fixedNote = '';
    render();
    askSheet({ action: 'budgetFixedSpend', since: `${month}-01`, until: `${month}-${String(last).padStart(2, '0')}` })
      .then((body) => {
        if (!Array.isArray(plan.fixed)) plan.fixed = [];
        const missed = [];
        const found = {};      // '항목|카테고리|상세SKU' → 광고비
        (body.rows || []).forEach((row) => {
          const spot = spotOf(row.product);
          if (!spot) { missed.push(row.campaign); return; }
          const key = `${row.item}|${spot.category}|${spot.sku}`;
          // salesAmt 는 부가세 별도다 — 그대로 쓴다 (공급가 기준).
          // 매체별 성과도 공급가라 두 화면 숫자가 그대로 맞는다.
        ...
        let made = 0;
        keys.forEach((key) => {
          const [item, category, sku] = key.split('|');
          let line = plan.fixed.find((one) => one.item === item
            && (one.category || '') === category && (one.sku || '') === sku);
          if (!line) {
            line = { id: uid(), category: category, sku: sku, item: item, plan: 0, used: 0 };
            plan.fixed.push(line);
            made += 1;
          }
          line.used = found[key];
        });
        fixedNote = keys.length
          ? `네이버에서 ${num(keys.length)}칸을 채웠습니다${made ? ` (줄 ${num(made)}개는 새로 만들었습니다)` : ''}.
             광고비는 <b>부가세를 뺀 공급가</b>입니다. 더 플렌더는 캠페인이 mini · MAX · PRO 로
             안 나뉘어 <b>상세 SKU 공통</b> 줄에 얹었습니다.`
          : '이 달에 쓴 파워링크 · 쇼핑검색 · 애드부스트 고정비가 없습니다.';
        // 판매채널 — 규칙에 걸린 줄의 실사용비를 채운다
        const chFilled = [];
        const chMany = [];
        const chNone = [];
        (body.channels || []).forEach((one) => {
          const mine = plan.rows.filter((row) =>
            String(row.channel || '').trim() === String(one.channel || '').trim());
          if (!mine.length) { if (one.spend) chNone.push(one.channel); return; }
          if (mine.length > 1) { chMany.push(`${one.channel} (${num(mine.length)}줄)`); return; }
          mine[0].used = one.spend;
          chFilled.push(`${one.channel} ${eok(one.spend)}`);
        });
        if (chFilled.length) {
          fixedNote += ` 판매채널로 ${escape(chFilled.join(' · '))} 을(를) 채웠습니다.`;
        }
        if (chMany.length) {
          fixedNote += ` <b>${escape(chMany.join(' · '))}</b> 는 같은 판매채널 줄이 여럿이라
            어느 줄 몫인지 알 수 없어 그대로 두었습니다 — 손으로 나눠 적어 주세요.`;
        }
        if (chNone.length) {
          fixedNote += ` ${escape(chNone.join(' · '))} 는 광고비가 있는데 그 판매채널을 쓴
            프로모션 줄이 없습니다.`;
        }
        (body.channelNotes || []).forEach((one) => {
          fixedNote += ` 매체를 못 읽었습니다 — ${escape(one)}`;
        });
        if (body.gfaNote) {
          fixedNote += ` 애드부스트는 못 받았습니다 — ${escape(body.gfaNote)}`;
        }
        if (missed.length) {
          fixedNote += ` 제품을 못 가린 캠페인 ${num(missed.length)}개는 빼 두었습니다 —
            ${escape(missed.slice(0, 3).join(', '))}${missed.length > 3 ? ' 외' : ''}.`;
        }
        fixedPulling = false;
        if (push) { pushSheet(); return; }     // 받은 김에 시트까지 올린다
        render();
        save();
      })
      .catch((reason) => {
        fixedPulling = false;
        fixedNote = `네이버에서 못 받았습니다 — ${escape(reason.message)}`;
        render();
      });
  };
  ...
  const monthStep = (want, by) => {
    const year = Number(String(want).slice(0, 4));
    const at = Number(String(want).slice(5, 7)) - 1 + by;
    const when = new Date(year, at, 1);
    return `${when.getFullYear()}-${String(when.getMonth() + 1).padStart(2, '0')}`;
  };

  // 새 달 칸의 기본값 — 가장 늦은 달의 다음 달 (대개 그걸 짜려고 누른다)
  const monthNext = () => {
    const list = monthList();
    return monthStep(list[list.length - 1] || thisMonth, 1);
  };

  const monthAdd = () => `<span class="bg-newmonth">
    <input type="month" data-bg="newMonth" value="${escape(monthNext())}" title="새로 짤 달">
    <button type="button" class="tool-copy-all" data-bg="addMonth">
  ...
  const autoLine = () => {
    ...
    const when = last && last.at ? clockText(last.at) : '';
    const done = !last ? ''
      : last.ok === false ? `지난번 실패 — ${escape(last.note || '')}`
        : `마지막 ${escape(when)}${last.how ? ` · ${escape(last.how)}` : ''}`
          + ` · 탭 ${num(last.tabs || 0)}개 · ${num(last.rows || 0)}줄`;
    if (auto.on) {
      return ` · 아침 <b>${escape(String(auto.hour || 11))}시</b>에 저절로 올라갑니다`
        + ` (${done || '아직 안 돌았습니다'})`;
    }
    // 표시는 꺼져 있어도 돈 적이 있으면 알려 준다 (손으로 트리거만 걸어 둔 경우)
    return done ? ` · ${done}` : '';
  };
  ...
  const noteLine = () => `<p class="perf-note bg-note${error ? ' bg-note-bad' : ''}">${error ? escape(error)
    : `${saved.at ? `마지막 저장 ${escape(new Date(saved.at).toLocaleString('ko-KR'))}${saved.by ? ` · ${escape(saved.by)}` : ''}`
      : '아직 저장한 적이 없습니다.'}${note ? ` · ${escape(note)}` : ''}${autoLine()}${fresh ? ''
      : ' · <b>이 브라우저에 담아 둔 판</b>을 먼저 보여 드리고 있습니다 — 시트 값이 오면 바뀝니다'}${editing
      ? ' · 고치는 중입니다. 손을 떼면 저절로 담기고, <b>저장하기</b> 를 누르면 보기로 돌아갑니다.' : ''}`}</p>`;
  ...
  const render = () => {
    ...
    budgetPlanView.innerHTML = `<div class="tool-head">
        <h2>월별 예산</h2>
        <p>이 화면에서 <b>직접 짭니다</b>. 아래 <b>프로모션</b>에 적으면 카테고리 · 총 예산이 저절로 합쳐 올라갑니다.
      ...
      <div class="tool-card">
        <div class="perf-filter">
          ...
          ${editing
    ? `<button type="button" class="tool-add" data-bg="save"><i data-lucide="save"></i>저장하기</button>`
    : `<button type="button" class="tool-add" data-bg="edit"${fresh ? '' : ' disabled'}>
        <i data-lucide="pencil"></i>${fresh ? '수정하기' : '시트에서 받는 중…'}</button>`}
          <button type="button" class="tool-copy-all" data-bg="sheetPull"
            title="매체에서 실사용비를 받아 채우고, 그대로 구글시트(월별예산 탭)에 올립니다"
    ...
    : pushing ? '구글시트에 올리는 중…' : '구글시트 받기'}</button>
      ...
      ${categoryCards()}
      ${fixedCard()}
      <div class="tool-list-head bg-title"><h3>프로모션</h3></div>
      ${events()}
`;
    lucide.createIcons();
  };
  ...
  const repaint = () => {
    plan.skus.forEach((sku) => {
      ...
      rowsOf(sku.name).forEach((row) => {
        const cost = rowShown(row);
        // 광고비 칸은 이제 적을 수 있는 칸이다. 수기로 안 적은 줄에는 셈한 값을
        // **적어 넣지 않고 흐린 글씨(placeholder)로** 보여 준다 — 값을 넣어 버리면
        // 사람이 적은 것과 셈한 것을 가릴 수 없다.
        const costAt = budgetPlanView.querySelector(`[data-bg="cost"][data-row="${row.id}"]`);
        if (costAt && !rowManual(row)) costAt.placeholder = cost ? commaNum(cost) : '자동';
      });
  ...
  const pushSheet = () => {
    ...
    pushing = true;
    render();
    return askSheet({ action: 'budgetPut', month, total: budgetAll(), plan, by: '' })
      .then((body) => {
        saved = { at: body.savedAt, by: saved.by };
        keepBudget(month, { plan, updatedAt: saved.at, updatedBy: saved.by });
        /* 두 곳에 담긴다 —
        ...
        if (body.tableNote) note += ` · 사람이 읽는 표는 못 그렸습니다 — ${body.tableNote}`;
        error = '';
      })
      .catch((reason) => { error = `구글시트에 못 올렸습니다 — ${reason.message}`; })
      .then(() => { pushing = false; render(); });
  };

  // 마지막으로 받은 판을 이 브라우저에 담아 둔다 (남에게는 안 보인다. 그림을 먼저 그리려는 것뿐이다)
  ...
  const readKept = (want) => {
    try {
      ...
      return null;   // 담아 두기는 거들기다. 안 되면 그냥 기다린다.
    }
  };
  ...
  const useBudget = (budget) => {
    plan = budget.plan && Array.isArray(budget.plan.skus) ? budget.plan : { skus: [], rows: [] };
    if (!Array.isArray(plan.rows)) plan.rows = [];
    saved = { at: budget.updatedAt || '', by: budget.updatedBy || '' };
  };
  ...
  const load = () => {
    ...
    note = '';
    fresh = false;
    // 담아 둔 판이 있으면 먼저 그린다. 기다리는 동안 빈 화면을 보지 않게.
    ...
    askSheet({ action: 'budgetGet', month })
      .then((body) => {
        ...
        auto = body.auto || null;
        useBudget(body.budget);
        keepBudget(body.month, body.budget);
        // 아직 아무것도 안 짠 달은 읽을 것이 없다. 바로 고치기로 연다.
        editing = !plan.skus.length && !fixedRows().length;
        fresh = true;
        status = 'ready';
        render();
      })
  ...
  const dropMonth = () => {
    ...
    const what = kinds || many || fixed || sum
      ? `SKU ${num(kinds)}개 · 프로모션 ${num(many)}줄` + (fixed ? ` · 고정비 ${num(fixed)}줄` : '')
        + (sum ? ` · 총 예산 ${eok(sum)}` : '')
      : '아직 아무것도 안 짠 달입니다';
  ...
  budgetPlanView.addEventListener('click', (event) => {
    ...
    if (what === 'edit') { editing = true; render(); return; }
    if (what === 'save') { save(true); return; }
    if (what === 'addMonth') {
      const box = budgetPlanView.querySelector('[data-bg="newMonth"]');
      const want = String((box && box.value) || '').slice(0, 7);
      if (!/^\d{4}-\d{2}$/.test(want)) { error = '달을 골라 주세요 (2026-10 처럼).'; paintNote(); return; }
      if (want === month) { error = `${want} 를 이미 보고 있습니다.`; paintNote(); return; }
      month = want;
      load();               // 시트에 없는 달이면 빈 판이 오고 고치기로 열린다
      return;
    }
    if (what === 'reload') { load(); return; }
    if (what === 'dropMonth') { dropMonth(); return; }
    if (what === 'sheetPull') {
      // 옛 판으로 올리면 남이 적은 것을 덮는다 — 시트에서 온 값일 때만 보낸다
      if (!fixedPulling && !pushing && fresh) fixedPull(true);
      return;
    }
    // 사람이 보는 것은 월별 예산 문서다. 그쪽을 먼저 연다.
    if (what === 'sheetOpen') { window.open(tableUrl || sheetUrl || SHEET_URL, '_blank', 'noopener'); return; }
    if (what === 'fixedAdd') {
      if (!Array.isArray(plan.fixed)) plan.fixed = [];
      plan.fixed.push({ id: uid(), category: '', sku: '', item: '', plan: 0, used: 0 });
      render();
      save();
      return;
    }
    if (what === 'fixedDrop') {
      plan.fixed = fixedRows().filter((one) => one.id !== hit.dataset.row);
      render();
      save();
      return;
    }
    if (what === 'sortRows') {
      sortRows(hit.dataset.sku);
      render();
      save();
      return;
    }
    if (what === 'media') {
      ...
      if (!row) return;
      const name = hit.dataset.media;
      const list = mediaOf(row);
      row.media = list.indexOf(name) >= 0 ? list.filter((one) => one !== name) : list.concat(name);
      hit.classList.toggle('is-on', row.media.indexOf(name) >= 0);
      save();
      return;
    }
    if (what === 'add') {
    ...
    if (what === 'dropSku') {
      ...
      if (many && !window.confirm(`${name} 의 프로모션 ${many}줄이 함께 지워집니다. 뺄까요?`)) return;
  ...
  // 숫자 칸을 떠나면 수식을 센 값으로 바꿔 적는다 ('=100*5' → '500').
  // 치는 동안에는 그대로 두어야 칠 수 있어서, 떠날 때(change) 한 번만 손본다.
  ...
  budgetPlanView.addEventListener('change', (event) => {
    ...
    if (!hit) return;
    const what = hit.dataset.bg;
    if (NUM_FIELDS.indexOf(what) >= 0) {
      const value = numValue(hit.value);
      if (value === null) return;          // 못 센 수식은 그대로 두고 빨간 칸을 남긴다
      hit.value = value ? commaNum(value) : '';
      hit.title = what === 'cost' ? '비우면 목표수량 × 목표 CPS 로 셉니다' : '';
      return;
    }
    if (what === 'month') { month = hit.value; load(); return; }
    if (what === 'withBrand') { withBrand = hit.checked; render(); return; }
    if (what === 'addSku') {
      const name = hit.value;
      if (!name || skuOf(name)) return;
      plan.skus.push({ name, category: categoryOf(name), brand: false, budget: 0 });
      render();
      save();
      return;
    }
    if (what === 'fixedCat' || what === 'fixedSku' || what === 'fixedItem') {
      const one = fixedRows().find((line) => line.id === hit.dataset.row);
      if (!one) return;
      if (what === 'fixedCat') {
        one.category = hit.value;
        // 카테고리를 바꿨는데 고른 SKU 가 그 안에 없으면 비운다
        const list = one.category
          ? (SKU_TREE.find(([name]) => name === one.category) || ['', []])[1] : [];
        if (one.category && list.indexOf(one.sku) < 0) one.sku = '';
      } else if (what === 'fixedSku') {
        one.sku = hit.value;
  ...
  /* 끌어 옮기기. 손잡이(⠿)만 끌 수 있게 두었다 — 줄 전체를 끌게 하면 칸 안의 글자를
     드래그로 고를 수 없게 된다. 떨어뜨린 자리의 위 · 아래는 줄의 가운데를 기준으로 가른다. */
  budgetPlanView.addEventListener('dragstart', (event) => {
    const grip = event.target.closest('[data-bg="grip"]');
    if (!grip) { event.preventDefault(); return; }
    dragId = grip.dataset.row;
    event.dataTransfer.effectAllowed = 'move';
    // 파이어폭스는 실어 보낸 것이 없으면 끌기를 시작하지 않는다
    try { event.dataTransfer.setData('text/plain', dragId); } catch (ignore) { /* 안 되면 그냥 */ }
    grip.closest('tr').classList.add('is-dragging');
  });

  budgetPlanView.addEventListener('dragover', (event) => {
    if (!dragId) return;
    const line = event.target.closest('tr[data-line]');
    if (!line || line.dataset.line === dragId) return;
    event.preventDefault();
    const box = line.getBoundingClientRect();
    const below = event.clientY > box.top + box.height / 2;
    budgetPlanView.querySelectorAll('.is-over-top, .is-over-bottom')
      .forEach((one) => one.classList.remove('is-over-top', 'is-over-bottom'));
    line.classList.add(below ? 'is-over-bottom' : 'is-over-top');
  });

  const dragDone = () => {
    dragId = '';
    budgetPlanView.querySelectorAll('.is-dragging, .is-over-top, .is-over-bottom')
      .forEach((one) => one.classList.remove('is-dragging', 'is-over-top', 'is-over-bottom'));
  };

  budgetPlanView.addEventListener('drop', (event) => {
    const line = event.target.closest('tr[data-line]');
    if (!dragId || !line) { dragDone(); return; }
    event.preventDefault();
    const moved = moveRow(dragId, line.dataset.line, line.classList.contains('is-over-bottom'));
    dragDone();
    if (!moved) return;
    render();
    save();
  ...
  budgetPlanView.addEventListener('dragend', dragDone);

  budgetPlanView.addEventListener('input', (event) => {
    const hit = event.target.closest('[data-bg]');
    if (!hit) return;
    const what = hit.dataset.bg;
    if (NUM_FIELDS.indexOf(what) < 0) return;
    // '=10*2' 처럼 적어도 받는다 (엑셀처럼). 수식일 때는 글자를 손대지 않는다 —
    // 콤마를 다시 붙이면 '=' 과 '*' 가 지워져 애초에 칠 수가 없다.
    const typed = hit.value;
    const formula = numFormula(typed);
    const value = numValue(typed);
    if (!formula) hit.value = value ? commaNum(value) : '';
    hit.classList.toggle('is-bad', value === null);
    if (value === null) return;      // 적다 만 수식이다. 앞서 센 값을 그대로 둔다
    hit.title = what === 'cost' ? '비우면 목표수량 × 목표 CPS 로 셉니다'
      : (formula ? `= ${commaNum(value)}` : '');
    if (what === 'cost') hit.classList.toggle('is-manual', value > 0);
    if (what === 'fixedPlan' || what === 'fixedUsed') {
      const one = fixedRows().find((line) => line.id === hit.dataset.row);
      if (!one) return;
      one[what === 'fixedPlan' ? 'plan' : 'used'] = value;
    } else if (what === 'skuBudget') {
  ...
  // 메뉴에 들어올 때 한 번만 부른다
  openViewOnce(budgetPlanView, load);
}
